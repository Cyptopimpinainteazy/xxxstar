...
mod tests {
    ...
    async fn test_cross_vm_settlement_with_packets() -> TestResult {
        ...
        // Step 5.1: Apply bridge mint with nonce/proof checks
        tracing::info!("Step 5.1: Apply bridge mint with proof+nonce checks");
        let proof_hash = [9u8; 32];
        let mint_ok = test_env.execute_bridge_mint_with_proof_nonce(
            alice.address(),
            lock_amount,
            1001,
            proof_hash,
        )?;
        assert!(mint_ok.success, "Bridge mint should succeed once");

        // Step 5.2: Replay must fail for same nonce/proof
        tracing::info!("Step 5.2: Ensure replay mint is rejected");
        let mint_replay = test_env.execute_bridge_mint_with_proof_nonce(
            alice.address(),
            lock_amount,
            1001,
            proof_hash,
        ...
        // Step 7: Verify bridge-specific invariants
        tracing::info!("Step 7: Verify bridge invariants");
        assert!(
            test_env.check_bridge_invariants()?,
            "Bridge invariant violated: wrapped exceeds native locked supply"
        );
...
struct MockState {
    ...
    seen_bridge_nonces: HashSet<u128>,
    seen_bridge_proof_hashes: HashSet<[u8; 32]>,
    packet_nonce: u64,
...
impl Default for MockState {
    fn default() -> Self {
        Self {
            ...
            seen_bridge_nonces: HashSet::new(),
            seen_bridge_proof_hashes: HashSet::new(),
            packet_nonce: 0,
...
impl TestEnvironment {
    ...
    pub fn execute_bridge_mint_with_proof_nonce(
        &self,
        recipient: [u8; 32],
        amount: u128,
        nonce: u128,
        ...
        if amount == 0 || proof_hash == [0u8; 32] {
            return Ok(ExecutionResult { success: false });
        }

        if state.seen_bridge_nonces.contains(&nonce)
            || state.seen_bridge_proof_hashes.contains(&proof_hash)
        {
            return Ok(ExecutionResult { success: false });
        }
        ...
        let bridge_minted_total: u128 = state
            ...
            .map(|(_, value)| *value)
            .sum();
        if bridge_minted_total.saturating_add(amount) > native_supply {
            return Ok(ExecutionResult { success: false });
        }

        state.seen_bridge_nonces.insert(nonce);
        state.seen_bridge_proof_hashes.insert(proof_hash);

        let key = (recipient, "EVM".to_string());
        let current = *state.balances.get(&key).unwrap_or(&0);
        state.balances.insert(key, current.saturating_add(amount));

        Ok(ExecutionResult { success: true })
    }

    pub fn check_bridge_invariants(&self) -> TestResult<bool> {
        let state = self.state.lock().expect("mock state lock poisoned");
        let native_supply = *state.supply_by_asset.get("NATIVE").unwrap_or(&0);
        let bridge_minted_total: u128 = state
            .balances
            ...
            .map(|(_, value)| *value)
            .sum();
        Ok(bridge_minted_total <= native_supply)
    }
    ...
    pub fn execute_lock_cross_vm(
        ...
        state
            .supply_by_asset
            .insert("NATIVE".to_string(), native_supply.saturating_add(_amount));
