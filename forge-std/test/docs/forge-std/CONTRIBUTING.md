...
When making modifications to the native cheatcodes or adding new ones, please make sure to run [`../../forge-std/scripts/vm.py`](./scripts/vm.py) to update the cheatcodes in the [`src/Vm.sol`](../../forge-std/src/Vm.sol) file.
...
It is possible that the resulting [`src/Vm.sol`](../../forge-std/src/Vm.sol) file will have some changes that are not directly related to your changes, this is not a problem.
