...
use crate::finality::{
    CrossVmFinalityVerifier, FinalityVerificationResult, ValidatorInfo, VmIdentifier,
};
...
impl FinalityFeed {
    ...
    pub fn poll(&self, current_time: u64) -> Result<VerifiedBatch, FeedError> {
        if !self.is_live() {
        ...
        for inc in &batch.proofs {
            let result: Result<FinalityVerificationResult, alloc::string::String> = match inc.vm {
                ...
                VmIdentifier::X3Vm => {
                    out.rejected.push("X3Vm proof not sourced externally".into());
                    continue;
                }
            };
            match result {
                ...
                Ok(fr) => out.rejected.push(
                    fr.error_message
                        .map(|b| alloc::string::String::from_utf8_lossy(&b).into_owned())
                        .unwrap_or_else(|| "invalid finality proof".into()),
                ),
...
mod tests {
    ...
    fn null_source_keeps_feed_off_even_if_enabled_flag_set() {
        ...
        assert!(!feed.is_live());
        assert_eq!(feed.poll(0).unwrap_err(), FeedError::NotConfigured);
    }
