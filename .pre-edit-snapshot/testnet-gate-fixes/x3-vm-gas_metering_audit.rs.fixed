...
mod tests {
    ...
    fn test_apply_audit_updates_cost() {
        ...
        // The table ships with GPU_MATMUL calibrated to the baseline benchmark
        // (2,500,000 ns). Applying an audit whose real measured cost EXCEEDS the
        // currently tabled value must raise it (audit updates cost upward).
        let original = table.cost("GPU_MATMUL").unwrap();

        // A later/higher measurement on slower reference hardware: 3,200µs.
        let audit = OpcodeGasAudit::from_measurement("GPU_MATMUL".to_string(), original, 3200.0);
