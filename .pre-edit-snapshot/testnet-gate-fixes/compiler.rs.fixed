...
fn simulation_runs_on_valid_intent() {
    ...
    // Test mode is required here: `new()` constructs the production simulator,
    // which fails closed (no real oracle wired => route_found=false) by design.
    // This test asserts the *route-finding* behavior of the simulator, which is
    // the synthetic test-mode contract.
...
fn simulation_detects_slippage_violation() {
    ...
    // Test mode so slippage estimation is actually evaluated (production mode
    // fails closed before reaching the slippage check).
    let simulator = IntentSimulator::test_mode();
    let mut intent = safe_bridge_swap_intent();
