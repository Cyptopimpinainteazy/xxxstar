...
impl FeeValidator {
    ...
    /// absolute 0..=MAX_BASIS_POINTS bounds (and non-zero if set, above the
    /// per-leg minimum) and the four legs summing to 10000. It does NOT apply
    /// the 50% per-leg cap `MAX_FEE_BPS`, which is reserved for
    /// `TreasurySplitConfig` buckets (a 5-way internal split where no single
    /// bucket dominating >50% is degenerate).
    pub fn validate_fee_config(config: &FeeConfig) -> Result<(), Vec<RevenueError>> {
        ...
        for bps in [
            ...
            config.referral_fee_bps,
            config.treasury_fee_bps,
        ] {
            if let Err(e) = Self::validate_fee_bps(bps) {
                errors.push(e);
            }
        }
    ...
    /// Validate a fee-config leg basis point value: 0..=10000 and, when
    /// non-zero, bounded below by `MIN_FEE_BPS`. (No 50% upper cap — see
    /// [`Self::validate_fee_config`].)
    fn validate_fee_bps(bps: u64) -> Result<(), RevenueError> {
        if bps > MAX_BASIS_POINTS {
            return Err(RevenueError::InvalidBasisPoints(bps));
        }
...
mod tests {
    ...
    fn test_invalid_fee_config() {
        ...
        let oversized_leg = FeeConfig::new(10_001, 0, 0, 0);
        assert!(oversized_leg.is_err());
    }
