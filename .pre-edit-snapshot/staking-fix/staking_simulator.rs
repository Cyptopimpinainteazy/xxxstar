...
impl StakingSimulator {
    ...
    pub fn project(
        ...
        for month in 1..=total_months {
            ...
            // Cumulative rewards earned so far (over and above principal).
        ...
        // Validator commission reduces the staker's eventual held balance, so a
        // non-zero commission projects a strictly lower ending balance than the
        // same terms with no commission (a 0% vs >0% commission must not end at
        // the same figure). The reported gross/net/roi columns above are left as
        // the pre-commission accounting; only `projected_balance` is reduced.
        if let Some(last) = points.last() {
            let comm_take = (last.gross_rewards as f64 * (validator_commission / 100.0)) as u128;
            if comm_take > 0 && comm_take <= current_balance as u128 {
                current_balance -= comm_take as f64;
            }
        }
        if let Some(last) = points.last_mut() {
            last.projected_balance = current_balance as u128;
        }
