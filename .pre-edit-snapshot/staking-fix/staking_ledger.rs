...
impl StakingLedger {
    ...
    pub fn unbond(&mut self, position_id: &str, amount: u128) -> Result<()> {
        ...
        // A position may hold several concurrent unbonding phases (the
        // `unbonding_phases` vector + `total_balance()` model it), so initiating
        // an additional unbond while one is already in progress is legitimate as
        // long as there is remaining active balance. Only a closed position
        // (fully claimed or exited) may not start a new unbond.
        if !matches!(
            position.status,
            PositionStatus::Active | PositionStatus::Locked | PositionStatus::Unbonding
        ) {
