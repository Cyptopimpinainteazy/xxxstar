...
export function SwarmCommand() {
  ...
  const pendingManualTasks = tasks.filter(
    (task) =>
      (task.status === "Pending" || task.status === "NeedsApproval") &&
      task.approval_required !== "None"
  );
  const latestTaskResult = latestTask ? `${latestTask.id} :: ${latestTask.status}` : "none";
  const blockerText = backendConnected ? "none" : "x3-swarm-api unavailable";
  const approvalText = latestTask?.approval_required ?? "None";

  const PanelMeta = () => (
    <div style={{ marginTop: 12, fontSize: 12, color: "#555" }}>
      <div>Mode: {health?.mode ?? "VIEW_ONLY"}</div>
      <div>Backend: {backendConnected ? "connected" : "not connected"}</div>
      <div>Latest task result: {latestTaskResult}</div>
      <div>Blockers: {blockerText}</div>
      <div>Approval requirement: {approvalText}</div>
    </div>
  );
  ...
  return (
    <div style={{ padding: 24, fontFamily: "Inter, sans-serif", lineHeight: 1.6 }}>
      ...
      <div style={{ display: "grid", gap: 16, gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))" }}>
        ...
        <div style={{ padding: 16, border: "1px solid #ddd", borderRadius: 12 }}>
          <h3>Memory Viewer</h3>
          <p>Memory entries are stored in data/agent-memory.</p>
        ...
        <div style={{ padding: 16, border: "1px solid #ddd", borderRadius: 12 }}>
          <h3>Feature Gaps</h3>
          <p>Feature gap report: reports/feature_gap_report.md</p>
          <p>Missing tests: reports/missing_tests_report.md</p>
          <PanelMeta />
        </div>
      </div>
    </div>
