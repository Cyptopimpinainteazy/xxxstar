...
export class BillingRegistry {
    dbPath;
    store = { plans: DEFAULT_PLANS, accounts: [] };
    activeWsConnections = new Map();
    constructor(dbPath) {
    ...
    async revokeAccount(apiKey, options) {
    ...
    async changeTier(apiKey, newTier) {
    ...
    async acquireConnectorSlot(apiKey) {
        const account = this.store.accounts.find((candidate) => candidate.apiKey === apiKey);
        if (!account) {
            throw new Error('INVALID_API_KEY');
        }
    ...
    async releaseConnectorSlot(apiKey) {
        const account = this.store.accounts.find((candidate) => candidate.apiKey === apiKey);
        if (!account) {
            return;
        }
        ...
        account.quotaRemaining.connectors = Math.min(plan.maxConnectors, account.quotaRemaining.connectors + 1);
        await this.persistStore();
    }
    async acquireWsConnection(apiKey) {
        const account = this.store.accounts.find((candidate) => candidate.apiKey === apiKey);
        if (!account) {
            throw new Error('INVALID_API_KEY');
        }
        this.refreshPeriodIfNeeded(account);
        const plan = this.store.plans[account.tier] ?? DEFAULT_PLANS.free;
        const active = this.activeWsConnections.get(apiKey) ?? new Set();
    ...
    async releaseWsConnection(apiKey, connectionId, connectedAtMs) {
...
export function extractApiKey(headers, requestUrl) {
