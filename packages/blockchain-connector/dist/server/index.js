...
export function startServer({ monitor, port = 9464, billingRegistry, connectorManager, }) {
    ...
    const server = http.createServer(async (req, res) => {
        ...
        if (path.startsWith('/health')) {
            const body = { version: '0.1.0', status: 'unknown' };
            if (monitor) {
                // expose top-level counts
                const map = monitor.statuses;
                if (map) {
                    const statuses = Array.from(map.values());
                    body.totalEndpoints = statuses.length;
                    body.healthy = statuses.filter((s) => s.healthy).length;
    ...
    const rejectUpgrade = (socket, statusCode, message) => {
        ...
        socket.write(`HTTP/1.1 ${statusCode} ${statusCode === 401 ? 'Unauthorized' : statusCode === 429 ? 'Too Many Requests' : 'Internal Server Error'}\r\n` +
    ...
    wsServer.on('connection', (ws, _req, session) => {
