...
export class TestRunner {
    ...
    getProfiles(chainId) {
        ...
        return TEST_PROFILES.filter((p) => p.supportedChains.includes("*") || p.supportedChains.includes(chainId));
    }
    /**
     * Run a test profile against a connector.
     */
    async runTest(connectorId, profileId) {
        const connector = this.manager.getConnector(connectorId);
        if (!connector)
            throw new Error(`Connector ${connectorId} not found`);
        const profile = TEST_PROFILES.find((p) => p.id === profileId);
        if (!profile)
            throw new Error(`Test profile ${profileId} not found`);
    ...
    async executeTest(connector, test) {
        const startMs = Date.now();
        const metrics = {};
        try {
            switch (test.type) {
                case "latency": {
                    const iterations = test.params.iterations || 100;
                    const latencies = [];
                ...
                case "throughput": {
                    const concurrency = Math.min(test.params.concurrency || 10, 20);
                    const durationSec = Math.min(test.params.durationSec || 10, 15);
                    let completed = 0;
                    let errors = 0;
                    const deadline = Date.now() + durationSec * 1000;
                    while (Date.now() < deadline) {
                        const batch = Array.from({ length: concurrency }, () => this.manager.getLatestBlock(connector.id).then(() => completed++).catch(() => errors++));
                ...
                case "reorg": {
                    ...
                    const depth = test.params.depth || 1;
                ...
                case "validation": {
                    ...
                    if (adapter?.getValidators) {
                        ...
                        const minCount = test.params.minValidators || 1;
                ...
                case "gpu": {
                    ...
                    const kernel = test.params.kernel;
                    const estimates = {
                        sha256: 10_100_000,
