...
export class BitcoinAdapter extends BaseChainAdapter {
    ...
    async getBlock(numberOrHash) {
        ...
        const raw = await this.httpGet(`block/${hash}`);
        return {
            hash: raw.id,
            number: raw.height,
    ...
    async getTransaction(hash) {
        const raw = await this.httpGet(`tx/${hash}`);
    ...
    async getMetrics() {
        ...
        try {
            ...
            const hashrate = await this.httpGet("mining/hashrate/1w").catch(() => null);
            return {
                ...base,
