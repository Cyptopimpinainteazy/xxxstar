...
export class NearAdapter extends BaseChainAdapter {
    ...
    async getBlock(numberOrHash) {
        const params = typeof numberOrHash === "number" || /^\d+$/.test(String(numberOrHash))
    ...
    parseBlock(raw) {
        ...
        return {
            ...
            txCount: raw?.chunks?.reduce((acc, c) => acc + (c.tx_root !== "11111111111111111111111111111111" ? 1 : 0), 0) ?? 0,
            size: 0,
            raw,
        };
    }
    async nearRpc(method, params) {
