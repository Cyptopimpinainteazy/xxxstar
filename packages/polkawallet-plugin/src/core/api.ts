...
function toPublicNetwork(network: ReturnType<typeof getCurrentNetwork>): X3Network {
  return `x3-${network}` as X3Network;
}
...
export class X3ChainApi extends EventEmitter<ApiEvents> {
  ...
  constructor(config: X3ChainConfig = {}) {
    ...
    this._config = {
      autoConnect: true,
      timeout: 30_000,
      network: toPublicNetwork(envConfig.network),
  ...
  get network(): X3Network {
    return this._config.network || 'x3-local';
  }
...
export async function createX3ApiFromEnv(): Promise<X3ChainApi> {
  ...
  return createX3Api({
    network: toPublicNetwork(config.network),
