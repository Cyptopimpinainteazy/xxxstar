...
## 2.1 Locked decisions and change control

The following decisions are locked for the X3Lang 1.0 readiness baseline:

- The Rust workspace under `x3-lang/` is the canonical compiler and VM path for X3Lang bytecode semantics.
- The Python files under `x3-lang/` remain the shipping intent DSL and are not silently replaced by the Rust compiler.
- Root `crates/x3-*` language components remain compatibility and runtime-integration surfaces; they must not define competing X3Lang grammar, type, IR, opcode, bytecode, or VM semantics.
- The source-to-execution order is parser, semantic analysis, IR/lowering, IR verification, register allocation, bytecode emission, bytecode verification, and VM execution.
- Consensus-sensitive opcode, bytecode, verifier, host-call, and gas changes require an explicit specification update, compatibility review, and conformance coverage before implementation is accepted.
- X3Lang 1.0 readiness cannot be declared from documentation or compilation alone; the relevant compiler, VM, determinism, malformed-input, resource, and integration gates must execute and pass.

Changing a locked decision requires updating this section and `docs/x3-lang/architecture-authority.md` in the same change, with the affected conformance and integration tests identified. Until then, new language work must follow this baseline.
...
## 5. Architectural Principle: One Canonical Execution Contract
...
## 6. Core Compiler Hardening Milestone
...
## 7. IR Contract and Verification
...
## 12. Property, Fuzz, and Differential Testing
...
## 13. X3-Native Type-System Direction
...
## 19. X3Lang 1.0 Definition of Done
