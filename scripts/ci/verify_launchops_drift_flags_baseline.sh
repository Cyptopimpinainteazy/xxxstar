#!/usr/bin/env bash
# LaunchOps drift-flag regression gate.
#
# `launchops scan` emits .launchops/drift_report.json, which includes a
# `drift_flags` array of critical regressions detected against the baseline
# branch (e.g. mainnet config changed without a review marker, assertions
# removed from tests). These flags are real launch blockers, but today's
# scorecard carries the current two as accepted pending Wave 0 work, so
# failing CI on the raw list would immediately red-light mainnet CI.
#
# This gate fingerprints each drift_flag and compares against a checked-in
# baseline (.launchops/drift_flags.baseline.json). CI fails only on:
...
#     UPDATE_BASELINE=1 bash scripts/ci/verify_launchops_drift_flags_baseline.sh
# and commit the updated baseline in the same PR; that puts the human review
# moment into the PR diff.
set -euo pipefail

drift=${DRIFT_FILE:-.launchops/drift_report.json}
baseline=${BASELINE_FILE:-.launchops/drift_flags.baseline.json}
update_baseline=${UPDATE_BASELINE:-0}

if [[ ! -f "$drift" ]]; then
  echo "::error::missing $drift; run 'cargo run -p launchops -- scan' first"
  exit 1
fi
if [[ ! -f "$baseline" ]]; then
...
python3 - "$drift" "$baseline" "$update_baseline" <<'PY'
import hashlib
import json
import sys

drift_path, baseline_path, update_flag = sys.argv[1], sys.argv[2], sys.argv[3]
...
if update_flag == "1":
    ...
    for fp, entry in scanned.items():
        ...
        merged.append({
            ...
            "accepted_reason": existing.get(
                "accepted_reason",
                "TODO: document why this drift flag is accepted at baseline update time",
            ),
        })
    ...
    with open(baseline_path, "w", encoding="utf-8") as handle:
        ...
        handle.write("\n")
    print(
        f"LaunchOps drift_flags baseline rewritten: {len(merged)} "
...
if new_flags:
    failed = True
    print(
        f"::error::{len(new_flags)} new LaunchOps drift_flag(s) not in "
...
if cleared_flags:
    failed = True
    print(
        f"::error::{len(cleared_flags)} LaunchOps drift_flag baseline "
...
print(
    f"LaunchOps drift_flags baseline: {len(scanned)} fingerprint(s) "
    "match baseline"
)
PY
