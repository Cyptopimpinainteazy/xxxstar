...
pub fn expand_genesis_config(def: &mut Def) -> proc_macro2::TokenStream {
	...
	match genesis_config_item {
		syn::Item::Struct(syn::ItemStruct { attrs, .. }) => {
			...
			// X3 patch: emit serde derive UNCONDITIONALLY (not gated on std) so that
			// `BuildGenesisConfig: MaybeSerializeDeserialize` is satisfied when sp-runtime/serde
			// is enabled in no_std builds. Field-level #[serde(skip)] attrs in source pallet
			// definitions are kept verbatim (no longer stripped).
			attrs.push(syn::parse_quote!(
