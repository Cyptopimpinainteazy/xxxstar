...
pub fn compute(func_idx: u32, module: &elements::Module) -> Result<u32, &'static str> {
    ...
    loop {
        ...
        match opcode {
            ...
            #[cfg(feature = "bulk")]
            Bulk(bulk) => match bulk {
                BulkInstruction::MemoryInit(_)
                | BulkInstruction::MemoryCopy
                | BulkInstruction::MemoryFill
                | BulkInstruction::TableInit(_)
                | BulkInstruction::TableCopy => {
                    // These instructions consume three i32 operands.
                    stack.pop_values(3)?;
                }
                BulkInstruction::MemoryDrop(_) | BulkInstruction::TableDrop(_) => {
                    // These instructions do not interact with the value stack.
                }
            },
