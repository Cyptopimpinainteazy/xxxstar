...
pub enum Instruction {
    ...
    Call(u32),
    // NOTE: Newer WASM encodings store a `tableidx` (varuint32) here instead of
    // a reserved u8=0. Using `u32` keeps us compatible with both encodings.
    ...
    fn deserialize<R: io::Read>(reader: &mut R) -> Result<Self, Self::Error> {
        ...
        Ok(match val {
            ...
            CALLINDIRECT => {
                ...
                // WASM MVP used a reserved u8=0 here, but newer encodings use a
                // `tableidx` (varuint32). Parsing as varuint32 supports both.
                let table_idx: u32 = VarUint32::deserialize(reader)?.into();
                CallIndirect(signature, table_idx)
            }
            DROP => Drop,
            SELECT => Select,
...
impl Serialize for Instruction {
    ...
    fn serialize<W: io::Write>(self, writer: &mut W) -> Result<(), Self::Error> {
        ...
        match self {
            ...
            CallIndirect(index, table_idx) => op!(writer, CALLINDIRECT, {
                VarUint32::from(index).serialize(writer)?;
                VarUint32::from(table_idx).serialize(writer)?;
            }),
            Drop => op!(writer, DROP),
