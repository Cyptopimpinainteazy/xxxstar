...
fn two_entry_policy(bps_a: u32, bps_b: u32) -> RevenueSplitPolicy {
    RevenueSplitPolicy {
        ...
        entries_len: 2,
        entries: [
            RevenueSplitEntry { destination: RevenueDestination::Treasury, share_bps: bps_a },
            RevenueSplitEntry {
...
fn validate_split_valid() {
    ...
    assert!(validate_split(&policy), "3000 + 7000 == 10000 must be valid");
}
...
fn revenue_split_policy_roundtrip() {
    let policy = RevenueSplitPolicy {
        ...
        entries_len: 3,
        entries: [
            RevenueSplitEntry { destination: RevenueDestination::Treasury, share_bps: 2_000 },
            RevenueSplitEntry {
                destination: RevenueDestination::DeveloperAccount,
                share_bps: 7_000,
            },
            RevenueSplitEntry { destination: RevenueDestination::ProtocolBurn, share_bps: 1_000 },
            empty_entry(),
            empty_entry(),
            empty_entry(),
            empty_entry(),
    ...
    let decoded =
        RevenueSplitPolicy::decode(&mut &encoded[..]).expect("decode must succeed");
...
fn placement_tier_encode() {
    assert_eq!(PlacementTier::Standard.encode(), vec![0u8], "Standard = index 0");
    assert_eq!(PlacementTier::Featured.encode(), vec![1u8], "Featured = index 1");
    assert_eq!(PlacementTier::Premium.encode(), vec![2u8], "Premium  = index 2");
}

/// 6. `RevenueDestination` encodes as expected SCALE byte values.
#[test]
fn revenue_destination_encode() {
    assert_eq!(RevenueDestination::Treasury.encode(), vec![0u8], "Treasury = index 0");
    assert_eq!(
        RevenueDestination::DeveloperAccount.encode(),
        vec![1u8],
        "DeveloperAccount = index 1"
    );
    assert_eq!(RevenueDestination::ProtocolBurn.encode(), vec![2u8], "ProtocolBurn = index 2");
    assert_eq!(RevenueDestination::LiquidityPool.encode(), vec![3u8], "LiquidityPool = index 3");
    assert_eq!(RevenueDestination::Stakers.encode(), vec![4u8], "Stakers = index 4");
}
