...
fn submit_chain_supply_report_stores_report() {
    new_test_ext().execute_with(|| {
        assert_ok!(Reconciliation::<Test>::submit_chain_supply_report(root(), 1, 1_000_000));
...
fn submit_chain_supply_report_overwrites_previous() {
    new_test_ext().execute_with(|| {
        assert_ok!(Reconciliation::<Test>::submit_chain_supply_report(root(), 1, 500));
        assert_ok!(Reconciliation::<Test>::submit_chain_supply_report(root(), 1, 999));
        assert_eq!(ChainSupplyReports::<Test>::get(1).unwrap().wrapped_supply, 999);
    });
}
...
fn set_canonical_supply_stores_value() {
    new_test_ext().execute_with(|| {
        assert_ok!(Reconciliation::<Test>::set_canonical_supply(root(), 1_000_000));
...
fn run_reconciliation_zero_divergence_when_exact_match() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Reconciliation::<Test>::set_canonical_supply(root(), canonical));
        assert_ok!(Reconciliation::<Test>::submit_chain_supply_report(root(), 1, canonical));
...
fn run_reconciliation_computes_correct_divergence_bps() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Reconciliation::<Test>::set_canonical_supply(root(), canonical));
        assert_ok!(Reconciliation::<Test>::submit_chain_supply_report(root(), 1, wrapped));
        assert_ok!(Reconciliation::<Test>::run_reconciliation(root()));
        let rec = LastReconciliation::<Test>::get().unwrap();
...
fn run_reconciliation_multi_chain_sums_correctly() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Reconciliation::<Test>::set_canonical_supply(root(), canonical));
        assert_ok!(Reconciliation::<Test>::submit_chain_supply_report(root(), 1, 1_000_000));
        assert_ok!(Reconciliation::<Test>::submit_chain_supply_report(root(), 2, 980_000));
        assert_ok!(Reconciliation::<Test>::run_reconciliation(root()));
        let rec = LastReconciliation::<Test>::get().unwrap();
...
fn run_reconciliation_above_tolerance_marks_degraded() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Reconciliation::<Test>::set_canonical_supply(root(), 1_000_000));
        assert_ok!(Reconciliation::<Test>::submit_chain_supply_report(root(), 1, 999_000));
        assert_ok!(Reconciliation::<Test>::run_reconciliation(root()));
        let rec = LastReconciliation::<Test>::get().unwrap();
        assert!(!rec.passed);
        assert_eq!(Reconciliation::<Test>::reconciliation_status(), ReconciliationStatus::Degraded);
    });
}
...
fn update_governance_power_stores_and_sums_correctly() {
    new_test_ext().execute_with(|| {
        assert_ok!(Reconciliation::<Test>::update_governance_power(root(), 1, 1_000));
        assert_ok!(Reconciliation::<Test>::update_governance_power(root(), 2, 2_000));
...
fn governance_power_divergence_alert_fires_when_above_threshold() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Reconciliation::<Test>::update_governance_power(root(), 1, 10_000));
        // Adding chain2 will trigger a divergence alert.
        assert_ok!(Reconciliation::<Test>::update_governance_power(root(), 2, 100));
        // Verify the alert event was deposited.
        let events = System::events();
        let has_alert = events.iter().any(|r| {
            matches!(
                r.event,
                RuntimeEvent::X3Reconciliation(crate::pallet::Event::GovernancePowerDivergenceAlert { .. })
            )
        });
...
fn governance_power_no_alert_when_within_threshold() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Reconciliation::<Test>::update_governance_power(root(), 1, 1_000));
        assert_ok!(Reconciliation::<Test>::update_governance_power(root(), 2, 1_001));
        let events = System::events();
        let has_alert = events.iter().any(|r| {
            matches!(
                r.event,
        ...
        assert!(!has_alert, "unexpected GovernancePowerDivergenceAlert event");
    });
}
...
fn aggregate_governance_power_resums_correctly() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Reconciliation::<Test>::aggregate_governance_power(signed(1)));
...
fn aggregate_governance_power_accessible_from_any_origin() {
    new_test_ext().execute_with(|| {
        assert_ok!(Reconciliation::<Test>::aggregate_governance_power(signed(99)));
    });
}
