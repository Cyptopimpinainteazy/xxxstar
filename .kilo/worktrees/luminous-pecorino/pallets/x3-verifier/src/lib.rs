...
pub mod pallet {
    ...
    use sp_std::vec::Vec;
    use x3_primitives::keyring::{KeyringVerifier, VerifyFromCompact};
    ...
    /// Verifiable claim with optional CID
    #[derive(Clone, Encode, Decode, DecodeWithMemTracking, TypeInfo, MaxEncodedLen, Debug)]
    #[scale_info(skip_type_params(T))]
    pub struct ClaimInfo<T: Config> {
        /// Unique claim ID (auto-incrementing)
        pub claim_id: u64,
        /// Content identifier (optional CID)
        pub cid: Option<BoundedVec<u8, T::MaxCidSize>>,
        /// Claim owner
        pub owner: T::AccountId,
        /// Block when claim was created
    ...
    pub trait Config: frame_system::Config {
        ...
        /// Maximum nonce value for replay protection
        #[pallet::constant]
        type MaxNonce: Get<u64>;

        /// Maximum CID size for claims
        #[pallet::constant]
    ...
    /// Used nonces for replay protection (nonce -> used status)
    #[pallet::storage]
    ...
    /// Claim information by claim ID
    #[pallet::storage]
    #[pallet::getter(fn claims)]
    pub type Claims<T: Config> =
        StorageMap<_, Blake2_128Concat, u64, ClaimInfo<T>, OptionQuery>;

    /// Next claim ID (auto-incrementing counter)
    #[pallet::storage]
    ...
    pub enum Event<T: Config> {
        ...
        ClaimCreated {
            claim_id: u64,
            cid: Option<BoundedVec<u8, T::MaxCidSize>>,
            owner: T::AccountId,
        },
        /// Claim verified
        ClaimVerified {
    ...
    impl<T: Config> Pallet<T> {
        ...
        /// Verify aggregate signature for batch receipt verification
        ///
        /// Each executor in the batch provides a secp256k1 ECDSA recoverable signature (65 bytes: r|s|v)
        /// over their individual receipt data. This allows a single extrinsic to verify multiple
        /// signers at once, reducing per-receipt overhead for the verifier.
        ///
        /// The message signed is: keccak256(job_id || input_hash || output_hash || state_root_before || state_root_after || gas_used || timestamp)
        /// Each executor must be registered and have a verified keyring (if keyring feature is enabled).
        #[pallet::call_index(6)]
        #[pallet::weight(T::WeightInfo::verify_aggregate_signature(executors.len() as u32))]
        pub fn verify_aggregate_signature(
            origin: OriginFor<T>,
            receipts: BoundedVec<ExecutionReceipt<T>, T::MaxStateChanges>,
        ) -> DispatchResult {
            ...
            ensure!(!receipts.is_empty(), Error::<T>::InvalidReceiptFormat);

            for receipt in receipts.iter() {
                // Verify the signature for each receipt
                Self::verify_receipt_signature(&receipt)?;

                // Verify executor is registered
                let executor_record =
                    Executors::<T>::get(&receipt.executor).ok_or(Error::<T>::ExecutorNotRegistered)?;
                ensure!(executor_record.active, Error::<T>::ExecutorNotActive);

                // If keyring verification is available, check that the executor's keyring is verified
                #[cfg(feature = "x3-keyring")]
                {
                    let keyring_id = sp_io::hashing::blake2_256(receipt.executor.encode().as_slice());
                    let keyring_id: [u8; 32] = keyring_id.into();
                    ensure!(
                        x3_primitives::keyring::KeyringVerifier::verify_keyring(&keyring_id),
        ...
        /// Verify multiple agents with nonce-based replay protection
        ///
        /// This extrinsic verifies that each agent's signature is valid and that the
        /// nonce has not been used before, preventing replay attacks.
        #[pallet::call_index(7)]
        #[pallet::weight(T::WeightInfo::verify_agents(agent_ids.len() as u32))]
        pub fn verify_agents(
            origin: OriginFor<T>,
            #[pallet::compact] nonce: u64,
            signatures: BoundedVec<(T::AccountId, BoundedVec<u8, ConstU32<65>>), T::MaxStateChanges>,
        ) -> DispatchResult {
            let who = ensure_signed(origin)?;
            ensure!(
                Self::verification_enabled(),
            ...
            ensure!(!UsedNonces::<T>::contains_key(&nonce), Error::<T>::InvalidSignature);

            // Verify nonce does not exceed maximum
            ensure!(nonce <= T::MaxNonce::get(), Error::<T>::InvalidReceiptFormat);

            // Verify at least one agent
            ensure!(!signatures.is_empty(), Error::<T>::InvalidReceiptFormat);
            ensure!(
                signatures.len() <= T::MaxStateChanges::get() as usize,
                Error::<T>::InvalidReceiptFormat
            );

            for (agent_id, signature) in signatures.iter() {
                // Verify the agent is a registered executor
                let executor_record =
                    Executors::<T>::get(agent_id).ok_or(Error::<T>::ExecutorNotRegistered)?;
                ensure!(executor_record.active, Error::<T>::ExecutorNotActive);

                // Verify using VerifyFromCompact trait
                // The message is keccak256(nonce || agent_id)
                let mut msg = Vec::new();
                msg.extend(&nonce.to_le_bytes());
                msg.extend(agent_id.encode());
                let msg_hash = keccak_256(&msg);

                // Use VerifyFromCompact to verify the signature
                // Convert signature to [u8; 65]
                let sig_bytes: [u8; 65] = signature
                    .as_slice()
                    .try_into()
                    .map_err(|_| Error::<T>::InvalidSignature)?;

                // Use the signature recovery approach via VerifyFromCompact
                let sig = ecdsa::Signature::from_slice(&sig_bytes)
                    .map_err(|_| Error::<T>::InvalidSignature)?;
                let expected = ecdsa::Public::from_slice(
                    &executor_record.account.encode()[..33]
                )
                .map_err(|_| Error::<T>::InvalidSignature)?;

                ensure!(
                    VerifyFromCompact::verify_from_compact(&sig, &msg_hash, &expected),
...
/// Verify multiple keyring attestors have signed a common message.
        ///
        /// This extrinsic verifies that a threshold of registered, active keyring attestors
        /// have each produced a valid ECDSA secp256k1 signature over the same message hash.
        /// It supports up to `MaxStateChanges` signatures in a single batch and enforces
        /// replay protection via a nonce consumed through the `verify_agents` pathway.
        ///
        /// The signed message is: `keccak256(nonce || message_hash)`
        /// where `message_hash` is provided by the caller and `nonce` is a compact-encoded
        /// replay-protection value.
        ///
        /// # Failure cases
        /// - `VerificationDisabled` — if the verifier pallet is not enabled.
        /// - `InvalidSignature` — if the nonce has already been used or exceeds `MaxNonce`,
        ///   the signature count is zero, or any individual signature is invalid.
        /// - `ExecutorNotRegistered` / `ExecutorNotActive` — if a signing attestor is not
        ///   a registered and active executor.
        /// - `InvalidReceiptFormat` — if the signature vector exceeds `MaxStateChanges`.
        #[pallet::call_index(8)]
        #[pallet::weight(T::WeightInfo::verify_multikey_proof(executor_keys.len() as u32))]
        pub fn verify_multikey_proof(
            origin: OriginFor<T>,
            #[pallet::compact] nonce: u64,
            message_hash: H256,
            mut executor_keys: BoundedVec<T::AccountId, T::MaxStateChanges>,
            mut signatures: BoundedVec<BoundedVec<u8, ConstU32<65>>, T::MaxStateChanges>,
        ) -> DispatchResult {
            let _who = ensure_signed(origin)?;
            ensure!(
                Self::verification_enabled(),
            ...
            // Verify at least one signature
            ensure!(!executor_keys.is_empty(), Error::<T>::InvalidReceiptFormat);
            ensure!(!signatures.is_empty(), Error::<T>::InvalidReceiptFormat);
            ensure!(
                executor_keys.len() <= T::MaxStateChanges::get() as usize,
                Error::<T>::InvalidReceiptFormat
            );
            ensure!(
                signatures.len() <= T::MaxStateChanges::get() as usize,
                ...
                executor_keys.len() == signatures.len(),
                Error::<T>::InvalidReceiptFormat
            );

            // Sort executor keys and corresponding signatures before verification
            Self::x3_crypto_sort(executor_keys.as_mut_slice(), signatures.as_mut_slice());

            // Construct the signed message: keccak256(nonce || message_hash)
            let mut msg = Vec::new();
            msg.extend(&nonce.to_le_bytes());
            msg.extend(message_hash.as_bytes());
            let msg_hash = keccak_256(&msg);

            // Verify each signature
            for (idx, executor_key) in executor_keys.iter().enumerate() {
                // Verify the executor is registered and active
                let executor_record =
                    Executors::<T>::get(executor_key).ok_or(Error::<T>::ExecutorNotRegistered)?;
                ensure!(executor_record.active, Error::<T>::ExecutorNotActive);

                // Verify the signature
                let sig_bytes: [u8; 65] = signatures[idx]
                    .as_slice()
                    .try_into()
                    .map_err(|_| Error::<T>::InvalidSignature)?;
            ...
            // Mark nonce as used (consumed via verify_agents pathway)
            UsedNonces::<T>::insert(&nonce, true);
        ...
        /// Sort executor keys and their corresponding signatures in-place by executor key.
        ///
        /// Takes two vectors of equal length: a list of AccountIds and a corresponding
        /// list of 65-byte ECDSA signatures. Sorts both vectors in-place according to
        /// the lexicographic ordering of the AccountId keys using an insertion sort
        /// algorithm.
        ///
        /// This extrinsic ensures deterministic ordering of multi-key signature sets,
        /// which is required for consistent verification results across the network.
        /// The sorted output prevents replay attacks that exploit signature ordering
        /// ambiguity in `verify_multikey_proof`.
        ///
        /// # Arguments
        /// * `executor_keys` - Vector of executor AccountIds (will be sorted in-place)
        /// * `signatures` - Vector of 65-byte ECDSA signatures (will be reordered to match sorted keys)
        ///
        /// # Errors
        /// - `InvalidReceiptFormat` — if the vectors have different lengths or exceed `MaxStateChanges`.
        #[pallet::call_index(9)]
        #[pallet::weight(T::WeightInfo::x3_crypto_sort(executor_keys.len() as u32))]
        pub fn x3_crypto_sort(
            origin: OriginFor<T>,
            mut executor_keys: BoundedVec<T::AccountId, T::MaxStateChanges>,
            mut signatures: BoundedVec<BoundedVec<u8, ConstU32<65>>, T::MaxStateChanges>,
        ) -> DispatchResult {
            ...
            // Perform insertion sort by executor key
            let len = executor_keys.len();
            for i in 1..len {
                for j in (1..=i).rev() {
                    if executor_keys[j - 1] > executor_keys[j] {
                        executor_keys.swap(j - 1, j);
                        signatures.swap(j - 1, j);
                    } else {
                        break;
                    }
                }
        ...
        /// Verify and register a claim with optional CID, signed by multiple attestor keys.
        ///
        /// This extrinsic verifies that each attestor key listed in `executor_keys` has
        /// signed the message `keccak256(nonce || cid || owner)`, confirming they are
        /// registered and active keyring attestors (validated via the X3KeyringApi).
        ///
        /// The claim is stored with a unique auto-incrementing `claim_id`.
        ///
        /// # Arguments
        /// * `cid` - Optional content identifier (bounded bytes)
        /// * `executor_keys` - Public keys (AccountIds) of the signers
        /// * `signatures` - Corresponding 65-byte ECDSA signatures (r|s|v)
        /// * `nonce` - One-time use value for replay protection (must be <= MaxNonce)
        ///
        /// # Errors
        /// - `VerificationDisabled` — if the verifier pallet is not enabled.
        /// - `InvalidSignature` — if the nonce has already been used or exceeds MaxNonce,
        ///   the signature count is zero, or any individual signature is invalid.
        /// - `ExecutorNotRegistered` / `ExecutorNotActive` — if a signing key is not
        ///   registered as an active attestor in the keyring pallet.
        /// - `InvalidReceiptFormat` — if the signature or key vectors exceed MaxStateChanges
        ///   or have mismatched lengths.
        #[pallet::call_index(10)]
        #[pallet::weight(T::WeightInfo::verify_create_claim(executor_keys.len() as u32))]
        pub fn verify_create_claim(
            origin: OriginFor<T>,
            cid: Option<BoundedVec<u8, T::MaxCidSize>>,
            #[pallet::compact] nonce: u64,
            mut executor_keys: BoundedVec<T::AccountId, T::MaxStateChanges>,
            ...
            // Verify at least one key/signature
            ensure!(!executor_keys.is_empty(), Error::<T>::InvalidReceiptFormat);
            ensure!(!signatures.is_empty(), Error::<T>::InvalidReceiptFormat);
            ensure!(
                executor_keys.len() <= T::MaxStateChanges::get() as usize,
            ...
            // Sort executor keys and signatures deterministically
            Self::x3_crypto_sort(executor_keys.as_mut_slice(), signatures.as_mut_slice());

            // Construct the signed message: keccak256(nonce || cid_bytes || owner)
            let mut msg = Vec::new();
            msg.extend(&nonce.to_le_bytes());
            if let Some(ref cid_bytes) = cid {
                msg.extend(cid_bytes.as_slice());
            }
            msg.extend(owner.encode());
            let msg_hash = keccak_256(&msg);

            // Verify each signature against registered keyring attestors
            for (idx, executor_key) in executor_keys.iter().enumerate() {
                // Check via runtime API that the executor key is a verified keyring attestor
                #[cfg(feature = "x3-keyring")]
                {
                    ensure!(
                        Self::is_keyring_attestor(executor_key),
                        Error::<T>::ExecutorNotRegistered
                    );
                }
            ...
            // Assign claim ID and store
            let claim_id = NextClaimId::<T>::get();
            NextClaimId::<T>::put(claim_id.saturating_add(1));
            ...
            // Reward each attestor for verification
            for executor_key in executor_keys.iter() {
                Self::reward_attestor_for_claim(executor_key)?;
            }
    ...
    impl<T: Config> Pallet<T> {
        ...
        /// Sort executor keys and their corresponding signatures in-place by executor key.
        ///
        /// Uses a simple insertion sort which is efficient for the small vector sizes
        /// expected in this pallet (bounded by `MaxStateChanges`).
        /// This ensures deterministic ordering of signatures for multi-key verification,
        /// preventing replay attacks that exploit signature ordering ambiguity.
        ///
        /// # Arguments
        /// * `executor_keys` - Mutable slice of executor keys (AccountId)
        /// * `signatures` - Mutable slice of signatures, will be reordered
        ///   to match the sorted executor_keys
        ///
        /// # Panics
        /// Panics if the lengths of executor_keys and signatures do not match.
        fn x3_crypto_sort(executor_keys: &mut [T::AccountId], signatures: &mut [BoundedVec<u8, ConstU32<65>>]) {
            let len = executor_keys.len();
            // Ensure both slices have the same length
            assert_eq!(len, signatures.len(), "executor_keys and signatures must have the same length");

            // Insertion sort by executor key
            for i in 1..len {
                for j in (1..=i).rev() {
                    if executor_keys[j - 1] > executor_keys[j] {
                        executor_keys.swap(j - 1, j);
