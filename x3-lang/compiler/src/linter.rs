...
impl X3Linter {
    pub fn new() -> Self {
        let known_chains: HashSet<String> = [
            "eth", "ethereum", "sol", "solana", "x3", "btc", "bitcoin", "utxo",
            "polygon", "arbitrum", "optimism", "base", "bsc", "avalanche",
            "arb", "avax", "ksm", "polkadot", "kusama", "sui", "aptos",
            "cosmos", "atom", "osmo", "starknet", "cardano", "ada", "ton",
            "fuel", "near", "stellar", "xlm",
    ...
    pub fn lint_program(&mut self, program: &Program) -> &[LintDiagnostic] {
        ...
        if has_intent && has_bridge && !has_refund_path {
            ...
            self.diagnostics.push(LintDiagnostic {
                severity,
                message: "intent with bridge operation missing refund path — add 'require refund_path' or 'on_fail refund'".into(),
        ...
        if has_intent && has_timeout && !has_nonce {
            self.diagnostics.push(LintDiagnostic {
                ...
                message: "intent with timeout missing nonce guard — add 'require nonce unused' for replay protection".into(),
                location: "program".into(),
            });
        }
            ...
            for &duration in &timeout_durations {
                if duration > 86400 {
                    self.diagnostics.push(LintDiagnostic {
                        ...
                        message: format!("unbounded deadline: {} blocks — reduce to ≤ 86400 for Mainnet", duration),
                        location: "intent".into(),
                    });
                }
            }
