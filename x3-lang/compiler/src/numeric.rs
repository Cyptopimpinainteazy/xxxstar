...
//! - signed/unsigned integer arguments are never implicitly coerced;
...
fn function_signatures(program: &Program) -> HashMap<String, Vec<Option<NumericKind>>> {
    ...
    for item in &program.items {
        if let Item::Function(function) = &item.node {
            let params = function.params.iter().map(|param| param.ty.as_ref().and_then(numeric_type)).collect();
...
fn numeric_type(ty: &TypeExpr) -> Option<NumericKind> {
    ...
    match name {
        "u8" | "u16" | "u32" | "u64" | "u128" | "u256" | "usize" => Some(NumericKind::Unsigned),
        "i8" | "i16" | "i32" | "i64" | "i128" | "i256" | "isize" => Some(NumericKind::Signed),
        _ => None,
    }
}
...
fn expression_numeric_kind(expr: &Expression) -> Option<NumericKind> {
    match expr {
        Expression::Literal(LiteralExpr::Int { suffix, .. }) => match suffix {
            None
            | Some(IntSuffix::U8)
            | Some(IntSuffix::U16)
            | Some(IntSuffix::U32)
            ...
            | Some(IntSuffix::U128)
            | Some(IntSuffix::U256)
            | Some(IntSuffix::Usize) => Some(NumericKind::Unsigned),
            Some(IntSuffix::I8)
            | Some(IntSuffix::I16)
            | Some(IntSuffix::I32)
            | Some(IntSuffix::I64)
            | Some(IntSuffix::I128)
            | Some(IntSuffix::I256)
            | Some(IntSuffix::Isize) => Some(NumericKind::Signed),
        },
        Expression::Unary { op: UnOp::Neg, expr } => {
            if matches!(expr.as_ref(), Expression::Literal(LiteralExpr::Int { .. })) {
                Some(NumericKind::Signed)
            } else {
                expression_numeric_kind(expr)
            }
        }
        _ => None,
    }
...
fn verify_statement(
    ...
    match stmt {
        Statement::Let { expr: Some(expr), .. } | Statement::Expr(expr) => verify_expression(expr, signatures, diagnostics),
...
fn verify_expression(
    ...
    match expr {
            ...
            if let Expression::Ident(name) = callee.as_ref() {
                if let Some(params) = signatures.get(name.as_str()) {
                    for (index, (param, arg)) in params.iter().zip(args.iter()).enumerate() {
                        if let (Some(expected), Some(actual)) = (param, expression_numeric_kind(arg)) {
                            if expected != &actual {
                                diagnostics.push(
                                    CompilerDiagnostic::error(
                                        DiagnosticCode::ArgumentTypeMismatch,
                                        format!(
                                            "argument {} to `{}` has incompatible signedness",
                                            index + 1,
                                            name.as_str()
                                        ),
                                        Span::DUMMY,
                                    )
                                    .with_help("X3Lang does not implicitly coerce signed and unsigned integer arguments"),
                                );
                            }
                        }
        ...
        | Expression::Try(expr) => verify_expression(expr, signatures, diagnostics),
        ...
        Expression::IfExpr { cond, then_block, else_block } => {
            verify_expression(cond, signatures, diagnostics);
            verify_block(then_block, signatures, diagnostics);
            if let Some(block) = else_block {
                verify_block(block, signatures, diagnostics);
