...
pub fn verify_trading_program(program: &Program, symbols: &TradingSymbols, mode: CompilationMode) -> Vec<X3Error> {
    ...
    for item in &program.items {
        if let Item::AtomicTrade(trade) = &item.node {
            errors.extend(verify_atomic_trade(trade, symbols, mode));
        }
    }
    errors
}
...
pub fn verify_atomic_trade(trade: &AtomicTradeDecl, symbols: &TradingSymbols, mode: CompilationMode) -> Vec<X3Error> {
    ...
    enforce_policy_bounds(policy, mode, &mut errors);
...
fn enforce_policy_bounds(policy: &x3_lang_ast::TradeRiskPolicy, mode: CompilationMode, errors: &mut Vec<X3Error>) {
    ...
    if deadline_is_zero(&policy.deadline) {
        errors.push(semantic_error(format!(
            "risk policy '{}' has a zero deadline",
            policy.name.as_str()
        )));
    }
