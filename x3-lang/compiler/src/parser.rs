...
impl<'a> Parser<'a> {
    ...
    fn parse_bridge_step(&mut self) -> Result<Statement, X3Error> {
        ...
        Ok(Statement::Bridge {
            ...
            from,
            to,
            amount: amount.unwrap_or_else(|| Expression::Literal(LiteralExpr::Int {
                value: 0,
                base: IntBase::Decimal,
                suffix: None,
            })),
            ...
            Tok::Ident(ref s) if s == "refund" => {
                ...
                if let Some(asset) = refund_asset {
                    let receiver = receiver
                        .map(|expr| expression_debug_string(&expr))
                        .unwrap_or_else(|| "sender".to_string());
                    FailureAction::Refund(Expression::Literal(LiteralExpr::String(Symbol::new(
                        &format!("{}.{}:{}", asset.chain.as_str(), asset.name.as_str(), receiver),
                    ))))
                } else {
                    FailureAction::Rollback
    ...
    fn parse_intent_use(&mut self) -> Result<Statement, X3Error> {
        ...
        Ok(Statement::Expr(Expression::Call {
            ...
            args: vec![
                Expression::Ident(Symbol::new(&target)),
                config,
            ],
        }))
    }
    ...
    fn parse_risk_policy_item(&mut self) -> Result<Item, X3Error> {
        ...
        Ok(Item::RiskPolicy(RiskPolicy { max_slippage, max_position }))
    }
    ...
    fn parse_proofs_required_item(&mut self) -> Result<Item, X3Error> {
        ...
        if required != "required" {
            return Err(parse_err("expected 'required' after 'proofs'".into(), Tok::Ident(required)));
        }
    ...
    fn parse_vm_decl_item(&mut self) -> Result<Item, X3Error> {
        ...
        Ok(Item::VmDecl(VmDecl { chain, adapter, finality }))
    }
    ...
    fn parse_finality_policy_item(&mut self) -> Result<Item, X3Error> {
        ...
        Ok(Item::FinalityPolicy(FinalityPolicy { mode, chain, requirement }))
    }
    ...
    fn parse_n_of_m(&mut self) -> Result<(u32, u32), X3Error> {
        ...
        let n = parts[0].parse::<u32>().map_err(|_| {
            parse_err(format!("invalid N_of_M numerator '{0}'", parts[0]), Tok::Eof)
        })?;
        let m = parts[1].parse::<u32>().map_err(|_| {
            parse_err(format!("invalid N_of_M denominator '{0}'", parts[1]), Tok::Eof)
        })?;
        Ok((n, m))
    }
