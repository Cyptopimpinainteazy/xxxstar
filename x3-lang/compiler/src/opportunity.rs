...
pub struct OpportunityConstraints {
    ...
    ///
    /// A set rather than a count, because "discover over these five chains" and
    /// "touch at most five chains" are different statements and only one of them is
    /// what a declaration like `chains = [x3, ethereum, base]` says. `None` leaves
    /// the graph unbounded; an empty set admits nothing, which is the honest reading
    /// of an empty declaration rather than a licence to search everything.
    ///
