...
impl RiskScorer {
    ...
    pub fn score_program(&self, program: &Program) -> RiskReport {
        ...
        // Chain risk score
        let chain_score: u32 = chains_used.iter()
        ...
        // Bridge risk score
        let bridge_score: u32 = bridges_used.iter()
        ...
        if !bridges_used.is_empty() {
            details.push(format!("bridge risk: {} (via {})", bridge_score, bridges_used.join(", ")));
        }
        ...
        // Timeout risk
        let timeout_score: u32 = if timeout_secs > 86400 { 30 }
            else if timeout_secs > 3600 { 15 }
            else if timeout_secs > 0 { 5 }
            else { 25 };
        ...
        // Slippage risk
        let slippage_score: u32 = if slippage_pct > 10 { 40 }
            else if slippage_pct > 5 { 20 }
