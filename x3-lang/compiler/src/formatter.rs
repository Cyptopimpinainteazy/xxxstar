...
impl X3Formatter {
    ...
    fn format_function(&mut self, f: &Function) {
        if f.is_async { self.write("async "); }
        self.write("fn ");
        self.write(f.name.as_str());
        self.write("(");
        for (i, p) in f.params.iter().enumerate() {
            if i > 0 { self.write(", "); }
            if p.is_mut { self.write("mut "); }
    ...
    fn format_statement(&mut self, stmt: &Statement) {
        match stmt {
            ...
            Statement::Lock { chain, asset, amount, from } => {
                self.write_indent();
                self.write("lock ");
            ...
            Statement::Swap { from, to, route, min_output, dex } => {
                self.write_indent();
            ...
            Statement::Bridge { via, from, to, amount, receiver, .. } => {
                self.write_indent();
                self.write("bridge ");
            ...
            Statement::ZkVerify { proof, public_input, key } => {
                self.write_indent();
    ...
    fn format_literal(&mut self, lit: &LiteralExpr) {
        match lit {
            LiteralExpr::Int { value, base, suffix: _ } => {
                match base {
