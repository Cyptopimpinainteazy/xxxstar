...
pub fn verify_refund_path_exists(ir: &X3IR, acc: &mut ErrorAccumulator) {
    ...
    for op in &ir.operations {
        if matches!(op, Operation::Bridge { .. } | Operation::Swap { .. } | Operation::Lock { .. }) {
            has_bridge = true;
        }
        ...
        InvariantRule {
            ...
            check_fn: |ir| {
                let refunds: Vec<&Operation> = ir
                    .operations
                    .iter()
                    .filter(|op| matches!(op, Operation::OnTimeout { action, .. }
                        if matches!(action, FailureAction::Refund { .. })
                    ))
                    .collect();
                if refunds.len() > 1 {
        ...
        InvariantRule {
            ...
            check_fn: |ir| {
                ...
                for op in &ir.operations {
                    ...
                    if found_claim && matches!(op, Operation::OnTimeout { action, .. }
                        if matches!(action, FailureAction::Refund { .. })
                    ) {
...
fn verify_single_relayer(ir: &X3IR, acc: &mut ErrorAccumulator) {
    let relayer_count = ir
        ...
        .count();
    if relayer_count == 0 {
        acc.add_error(err("mainnet: no relayer attestation declared — single-relayer is unsafe"));
        return;
    }
    for op in &ir.operations {
...
fn verify_solver_bond(ir: &X3IR, acc: &mut ErrorAccumulator) {
    let has_solver = ir
        .operations
        .iter()
        .any(|op| matches!(op, Operation::SolverBid { .. }));
    if !has_solver {
...
fn verify_known_assets(ir: &X3IR, acc: &mut ErrorAccumulator) {
    ...
    for op in &ir.operations {
        match op {
            ...
            Operation::Bridge { from_asset, to_asset, .. } => {
                if !known.contains(from_asset.to_uppercase().as_str()) {
                    acc.add_error(err(format!(
                        "mainnet: unknown from_asset '{from_asset}' in bridge"
                    )));
                }
                if !known.contains(to_asset.to_uppercase().as_str()) {
                    acc.add_error(err(format!(
                        "mainnet: unknown to_asset '{to_asset}' in bridge"
                    )));
                }
            }
            _ => {}
    ...
    for op in &ir.operations {
        ...
        } = op
        {
            if let Some(pct) = extract_slippage_percent(expr).filter(|p| *p > 5.0) {
                acc.add_error(err(format!(
                    "mainnet: slippage tolerance {pct}% exceeds maximum 5%"
                )));
            }
        }
    }
...
fn extract_slippage_percent(expr: &str) -> Option<f64> {
    let cleaned: String = expr.chars().filter(|c| c.is_ascii_digit() || *c == '.' || *c == '-' || *c == '+' || *c == 'e' || *c == 'E').collect();
...
pub fn compute_risk_score(ir: &X3IR) -> RiskScore {
    ...
    let unknown_chains: usize = ir
        ...
        .iter()
        .filter_map(|op| match op {
            Operation::Bridge { from_chain, to_chain, .. } => {
    ...
    let has_solver_bond = ir.operations.iter().any(|op| matches!(op, Operation::SolverBid { bond, .. } if *bond > 0));
    ...
    let has_liquidity_check = ir.operations.iter().any(|op| matches!(op, Operation::Require {
    ...
    let has_finality_check = ir.operations.iter().any(|op| matches!(op, Operation::Require {
        kind: crate::ir::RequireKind::Finality,
        ..
    }));
    ...
    let has_privacy = ir.operations.iter().any(|op| matches!(op, Operation::PrivacyCommit { .. }));
