...
fn test_parse_flagship_b52() {
    ...
    assert!(
        has_intent,
        "Flagship example must contain an intent declaration"
    );
    ...
    assert!(
        has_relayers,
        "Flagship example must contain a relayers declaration"
    );
    ...
    assert!(
        has_invariant,
        "Flagship example must contain invariant declarations"
    );
...
fn test_parse_staking_intent() {
    ...
    assert!(
        ...
        "Failed to parse staking_intent"
    );
}
...
fn test_check_mode_mainnet_rejects_unsafe_intent() {
    ...
    assert!(!errors.is_empty(), "mainnet mode should produce errors for unsafe intent");
    let all_msgs: String = errors.iter().map(|e| format!("{:?}", e)).collect::<Vec<_>>().join("\n");
    assert!(all_msgs.contains("slippage") || all_msgs.contains("refund"),
        "should mention slippage or refund in: {}", all_msgs);
}
...
fn test_lowering_vm_decl_emits_adapter_call() {
    ...
    assert!(
        ir.operations.iter().any(|op| matches!(op, Operation::VmAdapterCall { vm, .. } if vm == "evm")),
        "expected VmAdapterCall for evm"
    );
    assert!(
        ir.operations.iter().any(|op| matches!(op, Operation::ModeCheck { mode, .. } if mode == "finality")),
...
fn test_lowering_solver_market_emits_bid() {
    ...
    let ir = lower_program(&program, LowerCtx::new()).expect("should lower");
    assert!(
        ir.operations.iter().any(|op| matches!(op, Operation::SolverBid { bond: 1000, .. })),
...
fn test_lowering_relayer_swarm_emits_attest() {
    ...
    let ir = lower_program(&program, LowerCtx::new()).expect("should lower");
    assert!(
        ir.operations.iter().any(|op| matches!(op, Operation::RelayerAttest { quorum: (3, 5), relayers, .. } if relayers.len() == 3)),
...
fn test_lowering_rpc_quorum_emits_consensus() {
    ...
    let ir = lower_program(&program, LowerCtx::new()).expect("should lower");
    assert!(
        ir.operations.iter().any(|op| matches!(op, Operation::RpcConsensus { chain, require: (2, 3), .. } if chain == "solana")),
...
fn test_lowering_risk_policy_emits_score() {
    ...
    let ir = lower_program(&program, LowerCtx::new()).expect("should lower");
    assert!(
        ir.operations.iter().any(|op| matches!(op, Operation::RiskScore { score: 500, .. })),
...
fn test_lowering_privacy_emits_commit() {
    ...
    let ir = lower_program(&program, LowerCtx::new()).expect("should lower");
    assert!(
        ir.operations.iter().any(|op| matches!(op, Operation::PrivacyCommit { reveal_on, .. } if reveal_on == "commit")),
...
fn test_lowering_invariant_emits_check() {
    ...
    let ir = lower_program(&program, LowerCtx::new()).expect("should lower");
    assert!(
        ir.operations.iter().any(|op| matches!(op, Operation::InvariantCheck { name, .. } if name == "no_double_claim")),
...
fn test_lowering_finality_policy_emits_require() {
    ...
    let ir = lower_program(&program, LowerCtx::new()).expect("should lower");
    assert!(
        ir.operations.iter().any(|op| matches!(op, Operation::Require { kind: x3_lang_compiler::ir::RequireKind::FinalityExplicit, .. })),
...
fn test_lowering_proofs_emits_proof_required() {
    ...
    let program = parse_source(src).expect("should parse");
    let ir = lower_program(&program, LowerCtx::new()).expect("should lower");
    let proof_ops: Vec<_> = ir.operations.iter().filter(|op| matches!(op, Operation::ProofRequired { .. })).collect();
    assert_eq!(proof_ops.len(), 2, "expected 2 ProofRequired operations");
    assert!(
        proof_ops.iter().any(|op| matches!(op, Operation::ProofRequired { proof_type, .. } if proof_type == "source_lock_proof")),
        "expected ProofRequired for source_lock_proof"
    );
    assert!(
        proof_ops.iter().any(|op| matches!(op, Operation::ProofRequired { proof_type, .. } if proof_type == "fill_proof")),
