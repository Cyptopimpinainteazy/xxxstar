        ...
        match case.expect.as_str() {
            "accept" => match result {
                Ok((_program, _ir, errors)) => {
                    assert!(errors.is_empty(), "{}: expected acceptance, got semantic errors: {errors:?}", case.name);
                }
            ...
            "reject" => {
                let observed_code = match result {
                    ...
                    Ok((_program, _ir, errors)) => {
                        assert!(!errors.is_empty(), "{}: expected rejection, but compiler accepted source", case.name);
                        errors.iter().find_map(diagnostic_code_for_error).map(|code| code.as_str().to_owned())
                    }
                };

                if let Some(expected_code) = case.code.as_deref() {
                    assert_eq!(observed_code.as_deref(), Some(expected_code), "{}: wrong diagnostic code", case.name);
                }
            }
