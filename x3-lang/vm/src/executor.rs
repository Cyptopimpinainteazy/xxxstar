    ...
    let result = match decoded {
        ...
        CapabilityPayload::SubExec {
            ...
            if bytecode_hash.is_empty() {
                return Err(ExecError::Panic("sub exec: bytecode hash must be non-empty".to_string()));
            }
            if gas_limit == 0 {
                return Err(ExecError::Panic(
                    "sub exec: gas_limit must be positive".to_string(),
                ));
            }
        ...
        CapabilityPayload::SolverBid {
            ...
            if bond == 0 {
                return Err(ExecError::Panic(
                    "solver bid: bond must be positive".to_string(),
                ));
            }
            if fee.is_empty() {
                return Err(ExecError::Panic(
                    "solver bid: fee must be non-empty".to_string(),
                ));
            }
            let info = format!(
        ...
        CapabilityPayload::RelayerAttest {
            ...
            if quorum_denominator == 0 || quorum_numerator > quorum_denominator {
                return Err(ExecError::Panic(
                    "relayer attest: invalid quorum".to_string(),
                ));
            }
        ...
        CapabilityPayload::RpcConsensus {
            ...
            if chain.is_empty() {
                return Err(ExecError::Panic(
                    "rpc consensus: chain must be non-empty".to_string(),
                ));
            }
            if require_denominator == 0 || require_numerator > require_denominator {
                return Err(ExecError::Panic(
                    "rpc consensus: invalid require ratio".to_string(),
                ));
            }
        ...
        CapabilityPayload::RiskScore { score, category } => {
            if score > 100 {
                return Err(ExecError::Panic(
                    "risk score: score must be <= 100".to_string(),
                ));
            }
            if category.is_empty() {
                return Err(ExecError::Panic(
                    "risk score: category must be non-empty".to_string(),
                ));
            }
        ...
        CapabilityPayload::InvariantCheck {
            ...
            if name.is_empty() {
                return Err(ExecError::Panic(
                    "invariant check: name must be non-empty".to_string(),
                ));
            }
            vm.state.registers[0] = 1;
            Ok(vec![])
        ...
        CapabilityPayload::ProofRequired {
            ...
            if source.is_empty() {
                return Err(ExecError::Panic(
                    "proof required: source must be non-empty".to_string(),
                ));
            }
        ...
        CapabilityPayload::VmAdapterCall {
            ...
            if vm_name.is_empty() {
                return Err(ExecError::Panic(
                    "vm adapter call: vm must be non-empty".to_string(),
                ));
            }
            if adapter.is_empty() {
                return Err(ExecError::Panic(
        ...
        CapabilityPayload::ModeCheck {
            ...
            if mode.is_empty() {
                return Err(ExecError::Panic(
                    "mode check: mode must be non-empty".to_string(),
                ));
            }
        ...
        CapabilityPayload::PackageImport { path, alias: _ } => {
            if path.is_empty() {
                return Err(ExecError::Panic(
                    "package import: path must be non-empty".to_string(),
                ));
            }
            vm.state.registers[0] = 1;
            Ok(vec![])
        ...
        CapabilityPayload::RefundPolicy {
            ...
            if action.is_empty() {
                return Err(ExecError::Panic(
                    "refund policy: action must be non-empty".to_string(),
                ));
            }
