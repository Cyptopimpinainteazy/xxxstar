...
fn test_solver_bid_validates_bond() {
    ...
    assert_eq!(vm.state.bridge_ops[0].amount, 10000, "bond should be stored in bridge op amount");
}
...
fn test_vm_adapter_call_happy_path() {
    ...
    assert!(!vm.state.bridge_receipts.is_empty(), "should produce at least one receipt");
}
...
fn test_refund_policy_happy_path() {
    ...
    assert!(!vm.state.failure_handlers.is_empty(), "refund policy should register a failure handler");
}
