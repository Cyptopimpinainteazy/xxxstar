...
use x3_lang_compiler::{check_source, check_source_with_mode, compile_source, compile_to_ir, compile_with_mode, CompilationMode};
...
fn run(cli: Cli) -> Result<ExitCode, String> {
    ...
    match cli.command {
        ...
        Cmd::Fuzz {
            ...
            iterations,
            out,
        } => cmd_fuzz(&input, iterations, out.as_ref()),
        Cmd::Chaos {
            input,
            scenarios,
            out,
        } => cmd_chaos(&input, scenarios, out.as_ref()),
        Cmd::Deploy {
            input,
            target,
            mode,
...
fn cmd_lint(input: &PathBuf, mode_str: &str) -> Result<ExitCode, String> {
    ...
    if diagnostics.is_empty() {
        ...
        let error_count = diagnostics.iter().filter(|d| d.severity == x3_lang_compiler::linter::Severity::Error).count();
        let warning_count = diagnostics.iter().filter(|d| d.severity == x3_lang_compiler::linter::Severity::Warning).count();
...
"#
    ...
    println!("x3c fuzz: generated {} with {} iterations", out_path.display(), iterations);
    Ok(ExitCode::SUCCESS)
}
...
"#
    ...
    println!("x3c chaos: generated {} with {} scenarios", out_path.display(), scenarios);
    Ok(ExitCode::SUCCESS)
}

/// Compile and print a deployment plan.
fn cmd_deploy(input: &PathBuf, target: Option<&String>, mode: Option<&String>, out: Option<&PathBuf>) -> Result<ExitCode, String> {
    let source = read_source(input)?;
    ...
    println!("x3c deploy: target={target_str} mode={mode_str} ops={} bytes={}", ir.operations.len(), bytecode.len());
    Ok(ExitCode::SUCCESS)
}
...
fn cmd_inspect(input: &PathBuf, json: bool) -> Result<ExitCode, String> {
    ...
    if ext == "x3b" {
        ...
        if json {
            ...
            println!("{}", serde_json::to_string_pretty(&dis_json).map_err(|e| format!("serialization: {e}"))?);
        } else {
            ...
            for stmt in &intent.body.stmts {
                match stmt {
                    ...
                    x3_lang_ast::ast::Statement::Bridge { via, from, to, .. } => {
                        route_steps.push(format!("bridge {} {} -> {}", via.as_str(), from.name.as_str(), to.name.as_str()));
                    ...
                    x3_lang_ast::ast::Statement::Swap { from, to, dex, .. } => {
                        ...
                        route_steps.push(format!("swap {} {} -> {}", dex_str, from.name.as_str(), to.name.as_str()));
                    }
    ...
    if json {
        println!("{}", serde_json::to_string_pretty(&metadata).map_err(|e| format!("serialization failed: {e}"))?);
    } else {
...
fn cmd_verify(intent: &PathBuf, proof: &PathBuf) -> Result<ExitCode, String> {
    ...
    // Parse proof as JSON
    let proof_val: serde_json::Value = serde_json::from_str(&proof_data)
    ...
    if all_verified {
        ...
        print_error(&format!("intent '{}' verification failed — {}", draft.name, reasons.join("; ")));
        Ok(ExitCode::from(1))
    }
}
...
fn cmd_audit(input: &PathBuf, mode_str: &String, out: Option<&PathBuf>) -> Result<ExitCode, String> {
    ...
    let has_intent = program.items.iter().any(|item| matches!(&item.node, x3_lang_ast::ast::Item::IntentDecl(_)));
    if has_intent {
    ...
    let has_timeout = program.items.iter().any(|item| {
        if let x3_lang_ast::ast::Item::IntentDecl(intent) = &item.node {
            intent.body.stmts.iter().any(|s| matches!(s, x3_lang_ast::ast::Statement::OnTimeout { .. }))
        } else {
            false
        }
    });
        ...
        if let x3_lang_ast::ast::Item::IntentDecl(intent) = &item.node {
            for stmt in &intent.body.stmts {
                if let x3_lang_ast::ast::Statement::Bridge { from, .. } = stmt {
                    let known = ["eth", "ethereum", "sol", "solana", "x3", "btc", "bitcoin", "polygon", "arbitrum", "optimism", "base", "bsc", "avalanche"];
                    let chain = from.chain.as_str().to_ascii_lowercase();
                    if !known.contains(&chain.as_str()) {
                        issues.push(format!("[WARN] unknown chain '{}' in bridge — verify support", from.chain.as_str()));
                    }
                }
            }
        }
    ...
    let has_vm = program.items.iter().any(|item| matches!(&item.node, x3_lang_ast::ast::Item::VmDecl(_)));
    if has_vm {
    ...
    let has_solver_market = program.items.iter().any(|item| matches!(&item.node, x3_lang_ast::ast::Item::SolverMarket(_)));
    ...
    let has_relayer_swarm = program.items.iter().any(|item| matches!(&item.node, x3_lang_ast::ast::Item::RelayerSwarm(_)));
    ...
    let has_rpc_quorum = program.items.iter().any(|item| matches!(&item.node, x3_lang_ast::ast::Item::RpcQuorum(_)));
    if has_rpc_quorum {
    ...
    let has_risk_policy = program.items.iter().any(|item| matches!(&item.node, x3_lang_ast::ast::Item::RiskPolicy(_)));
    if has_risk_policy {
    ...
    let has_privacy_block = program.items.iter().any(|item| matches!(&item.node, x3_lang_ast::ast::Item::PrivacyBlock(_)));
    ...
    let has_invariant = program.items.iter().any(|item| matches!(&item.node, x3_lang_ast::ast::Item::InvariantDecl(_)));
    if has_invariant {
    ...
    let has_proofs_required = program.items.iter().any(|item| matches!(&item.node, x3_lang_ast::ast::Item::ProofsRequired(_)));
    ...
    let has_finality_policy = program.items.iter().any(|item| matches!(&item.node, x3_lang_ast::ast::Item::FinalityPolicy(_)));
    ...
    let has_target = program.items.iter().any(|item| matches!(&item.node, x3_lang_ast::ast::Item::VmTarget(_)));
    if has_target {
...
fn cmd_refund(intent_hash: &str, check_only: bool) -> Result<ExitCode, String> {
            ...
            for stmt in &intent.body.stmts {
                match stmt {
                    x3_lang_ast::ast::Statement::OnTimeout { duration, action } => {
                        has_timeout = true;
                        if let x3_lang_ast::ast::Expression::Literal(x3_lang_ast::ast::LiteralExpr::Int { value, .. }) = duration {
    ...
    if check_only {
        ...
        println!("Refund path: {}", if refund_path_exists { "present" } else { "missing" });
...
"#;
    ...
    std::fs::write(project_dir.join("src").join("main.x3"), &main_x3)
        .map_err(|e| format!("write main.x3: {e}"))?;
...
"#,
    ...
    println!("  {}", project_dir.join("tests").join(format!("test_{name}.rs")).display());
    println!();
    println!("Next steps:");
...
fn cmd_plan(input: &PathBuf, show_route: bool, json: bool) -> Result<ExitCode, String> {
    ...
    if json {
        println!("{}", serde_json::to_string_pretty(&plan).map_err(|e| format!("serialization failed: {e}"))?);
    } else {
