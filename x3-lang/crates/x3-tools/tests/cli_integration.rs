...
fn test_cli_refund_on_timeout_refund() {
    ...
    let output = x3c()
        ...
        .expect("x3c refund");
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("Refund") || stdout.contains("refund"), "refund command should find a refund path");
}
...
fn test_cli_inspect_json_flag() {
    let src = write_fixture("cli_inspect_json.x3", r#"intent test_json {
