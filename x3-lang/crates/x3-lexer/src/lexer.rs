use crate::token::{Delimiter, IntBase, Keyword, Literal, Token, TokenKind};
        ...
        while i < bytes.len() {
            ...
            match ch {
                ...
                c if c.is_ascii_digit() => {
                    ...
                    let value = source[start..i].parse::<u128>().unwrap_or(0);
                    tokens.push(token(
                ...
                ')' => tokens.push(token(
                    TokenKind::Delimiter(Delimiter::CloseParen),
                    start,
                    i,
                    file_id,
                )),
...
fn token(kind: TokenKind, start: usize, end: usize, file_id: u32) -> Token {
    Token::new(
        kind,
        Span::new(BytePos(start as u32), BytePos(end as u32), file_id),
    )
}
