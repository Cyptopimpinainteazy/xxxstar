...
pub fn create_router(
    ...
    Router::new()
        ...
        .route(
            "/api/public/funding-swarm/grants",
            get(get_funding_swarm_grants),
        )
        .route(
            "/api/public/funding-swarm/timeline",
...
async fn get_funding_swarm_scoreboard(
    State(state): State<AppState>,
) -> Result<impl IntoResponse, GatewayError> {
    let scoreboard: FundingSwarmScoreboard = state.db.get_funding_swarm_scoreboard().await?;
    Ok(Json(scoreboard))
}

async fn get_funding_swarm_grants(
    State(state): State<AppState>,
    Query(pagination): Query<Pagination>,
) -> Result<impl IntoResponse, GatewayError> {
    let grants: Vec<FundingSwarmGrant> = state
        .db
        .list_funding_swarm_grants(bounded_limit(pagination.limit), pagination.offset.max(0))
        .await?;
    Ok(Json(grants))
}

async fn get_funding_swarm_timeline(
    State(state): State<AppState>,
    Query(pagination): Query<Pagination>,
) -> Result<impl IntoResponse, GatewayError> {
    let timeline: Vec<FundingSwarmTimelineItem> = state
        .db
