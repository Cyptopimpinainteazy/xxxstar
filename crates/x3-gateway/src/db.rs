...
#[derive(Debug, Clone, Serialize, Deserialize, FromRow)]
pub struct FundingSwarmTimelineItem {
    pub item_id: String,
...
#[derive(Debug, Clone, Serialize, Deserialize, FromRow)]
pub struct FundingSwarmScoreboard {
    pub total_grants: i64,
    pub open_grants: i64,
    pub submitted_proposals: i64,
    pub approved_grants: i64,
    pub funded_grants: i64,
    pub total_awarded_usd: f64,
...
impl Database {
    ...
    pub async fn get_funding_swarm_scoreboard(&self) -> Result<FundingSwarmScoreboard> {
        let row = sqlx::query_as(
            "SELECT
                COUNT(*)::BIGINT AS total_grants,
                COUNT(*) FILTER (WHERE status = 'open')::BIGINT AS open_grants,
                COUNT(*) FILTER (WHERE status = 'submitted')::BIGINT AS submitted_proposals,
                COUNT(*) FILTER (WHERE status = 'approved')::BIGINT AS approved_grants,
                COUNT(*) FILTER (WHERE status = 'funded')::BIGINT AS funded_grants,
                COALESCE(SUM(amount_usd) FILTER (WHERE status = 'funded'), 0)::DOUBLE PRECISION AS total_awarded_usd,
                (SELECT MAX(created_at) FROM funding_swarm_events WHERE visibility = 'public') AS last_event_at
             FROM funding_swarm_grants",
        )
        .fetch_one(&self.pool)
        .await?;
    ...
    pub async fn list_funding_swarm_grants(
        &self,
        limit: i64,
        offset: i64,
    ...
    pub async fn list_funding_swarm_timeline(
        &self,
        limit: i64,
        offset: i64,
    ) -> Result<Vec<FundingSwarmTimelineItem>> {
        sqlx::query_as(
            "SELECT
                event_id AS item_id,
             ...
             FROM funding_swarm_events
             WHERE visibility = 'public'
             UNION ALL
             SELECT
                publication_id AS item_id,
                'publication'::TEXT AS item_type,
                kind AS title,
                status,
                grant_id,
                jsonb_build_object('title', title, 'body', body) AS data,
                COALESCE(published_at, created_at) AS occurred_at
             FROM funding_swarm_publications
