...
impl MediaRpc {
    ...
    pub fn with_dispatcher(mut self, d: std::sync::Arc<std::sync::Mutex<()>>) -> Self {
        self.dispatcher = Some(d);
        self
    }
    pub fn with_reputation_repo(
        mut self,
        repo: std::sync::Arc<dyn crate::reputation::ReputationRepo>,
    ) -> Self {
...
impl MediaApi for MediaRpc {
    ...
    async fn get_job_status(&self, job_id: String) -> Result<MediaJobStatus, String> {
        Ok(MediaJobStatus {
            ...
            assigned_node_id: Some("node-123".to_string()),
    ...
    async fn get_job_result(&self, job_id: String) -> Result<MediaJobResult, String> {
        Ok(MediaJobResult {
            ...
            data: json!({"output":"s3://bucket/output.mp4"}),
    ...
    async fn get_node_reputation(&self, node_id: String) -> Result<serde_json::Value, String> {
        if self.reputation_repo.is_some() {
            ...
            let factors = crate::reputation::ReputationFactors {
                uptime_fraction: 0.98,
                success_rate: 0.95,
                watchdog_pass_rate: 0.97,
    ...
    async fn get_network_stats(&self) -> Result<NetworkStats, String> {
        Ok(NetworkStats {
            ...
            offline_nodes: 0,
            total_vram_gb: 184,
            available_vram_gb: 140,
            total_jobs_completed: 2048,
        })
    }
    async fn get_pending_jobs(&self) -> Result<Vec<MediaJobStatus>, String> {
        Ok(vec![])
    ...
    async fn estimate_job(&self, _request: MediaJobRequest) -> Result<serde_json::Value, String> {
        Ok(json!({}))
    }
    async fn get_swarm_capacity(&self) -> Result<serde_json::Value, String> {
        Ok(json!({}))
...
mod tests {
    ...
    async fn test_reputation_endpoints_stub() {
        let repo: std::sync::Arc<dyn crate::reputation::ReputationRepo> =
            std::sync::Arc::new(crate::reputation::InMemoryRepo::default());
        ...
        let wallet_rep = rpc
            .get_wallet_reputation("0xabc".to_string())
            .await
            .unwrap();
        assert!(wallet_rep.to_string().contains("wallet"));

        let slash = crate::reputation::SlashingEvent {
            id: 1,
    ...
    async fn test_reputation_endpoints_with_pg_repo() {
        ...
        let pool = sqlx::PgPool::connect(&database_url)
            .await
            .expect("could not connect to postgres for test");
        ...
        sqlx::query(cleanup_seq_reputation)
            .execute(&pool)
            .await
            .expect("migration failed");
        sqlx::query(cleanup_seq_slashing)
            .execute(&pool)
            .await
            .expect("migration failed");
        sqlx::query(create_reputation)
            .execute(&pool)
            .await
            .expect("migration failed");
        sqlx::query(create_slashing)
            .execute(&pool)
            .await
            .expect("migration failed");
        ...
        let empty = rpc
            .get_slashing_events("this-wallet-does-not-exist".to_string())
            .await
            .unwrap();
        ...
        let slash1 = crate::reputation::SlashingEvent {
            id: 0,
            wallet_address: "0xedge".to_string(),
            node_id: None,
            severity: 0.2,
            slash_amount: 2.0,
            recurrence_count: 1,
        ...
        let slash2 = crate::reputation::SlashingEvent {
            id: 0,
            wallet_address: "0xedge".to_string(),
            node_id: None,
            severity: 0.5,
