...
impl ReputationRepo for InMemoryRepo {
    ...
    async fn get_reputation_events(&self, wallet: &str) -> Result<Vec<ReputationEvent>, String> {
        ...
        Ok(g.iter()
            .filter(|e| e.wallet_address == wallet)
            .cloned()
            .collect())
    }
    async fn get_slashing_events(&self, wallet: &str) -> Result<Vec<SlashingEvent>, String> {
...
pub fn compute_wallet_reputation(
    node_reps: &[f64],
    funding_factor: f64,
    social_factor: f64,
) -> f64 {
    ...
    (median * (1.0 + f_funding + f_social)).clamp(0.0, 1.0)
}
    ...
    impl ReputationRepo for PgRepo {
        async fn insert_reputation_event(&self, ev: ReputationEvent) -> Result<(), String> {
            ...
            sqlx::query(query)
                ...
                .bind(&ev.event_type)
                .bind(ev.delta)
                .bind(ev.prev_reputation)
                .bind(ev.new_reputation)
...
async fn test_pgrepo_reputation_events_roundtrip() {
    ...
    repo.insert_reputation_event(ev.clone())
        .await
        .expect("insert failed");
    let found = repo
        .get_reputation_events("0xroundtrip")
        .await
        .expect("query failed");
...
async fn test_slashing_appeal_lifecycle_and_recurrence() {
    ...
    let s = SlashingEvent {
        id: 0,
        wallet_address: "0xappeal".to_string(),
        node_id: None,
        severity: 0.4,
        slash_amount: 4.0,
        recurrence_count: 1,
        evidence_hash: None,
        occurred_at: now,
        appeal_status: "pending".to_string(),
    };
