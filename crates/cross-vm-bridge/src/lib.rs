...
// External-chain finality verifier: EVM (BLS aggregate) + SVM (ed25519/lockout)
// proofs with cross-VM consistency. See `finality.rs` for invariants.
pub mod finality;
...
// External finality header-feed seam: default-OFF bridge between an
// operator-supplied relayer/light-client (`HeaderSource`) and the `finality`
// verifiers. Std-only; never performs network I/O itself. See `finality_feed.rs`.
#[cfg(feature = "std")]
pub mod finality_feed;
...
pub struct NoOpDispatcher {
    evm_escrow: [u8; 20],
    svm_escrow: [u8; 32],
    // Testnet balances reported by `get_evm_balance` / `get_svm_balance`.
    // Default 0 (an unfunded fixture represents a genuine underfunding state);
    // `testnet()` funds generously so positive 2PC lifecycle tests can acquire
    // source/destination locks (which now enforce a balance check).
    evm_balance: u128,
    svm_balance: u64,
}
...
impl NoOpDispatcher {
    ...
    /// Set the simulated EVM balance reported for every address.
    pub fn with_evm_balance(mut self, balance: u128) -> Self {
        self.evm_balance = balance;
        self
    }

    /// Set the simulated SVM balance reported for every address.
    pub fn with_svm_balance(mut self, balance: u64) -> Self {
        self.svm_balance = balance;
        self
    }

    /// Create a funded testnet dispatcher with placeholder escrow addresses.
    /// Accounts are seeded with a generous balance so that 2PC lock acquisition
    /// (which checks `balance >= amount`) succeeds for lifecycle tests.
    pub fn testnet() -> Self {
        Self::new(
            [
        ...
        .with_evm_balance(1_000_000_000_000)
        .with_svm_balance(1_000_000_000_000)
    }
}
