...
pub struct EvmFinalityVerifier {
    // Chain identifier retained for caller-side pinning. In-flow, chain identity
    // is already bound via `genesis_hash` (Step 1 of verify_finality_proof),
    // which uniquely identifies an EVM chain. Kept as a public-API identity
    // anchor for the upstream relayer/pallet wiring.
    #[allow(dead_code)]
    chain_id: u64,
    genesis_hash: H256,
    current_epoch: u64,
...
impl EvmFinalityVerifier {
    ...
    fn verify_bls_aggregate(
        ...
        if aggregated_sig.len() != 96 {
            return Err(DispatchError::Other(
                "BLS aggregate signature must be 96 bytes",
            ));
        }

        let validator_count = public_keys.len();
        let required_bitfield_len = validator_count.div_ceil(8);
        if bitfield.len() != required_bitfield_len {
            return Err(DispatchError::Other(
                "Invalid bitfield length for validator set",
            ));
        }
        ...
        for (i, &pk_bytes) in public_keys.iter().enumerate() {
            ...
            if pk_bytes.len() != 48 {
                return Err(DispatchError::Other("Public key must be 48 bytes"));
            }
        ...
        // `fast_aggregate_verify` returns a `BLST_ERROR`; success requires BLST_SUCCESS.
        // It expects `&[&PublicKey]`, so collect references from the owned keys.
        let pk_refs: Vec<&BlstPublicKey> = pub_keys.iter().collect();
        let error = agg_sig.fast_aggregate_verify(
            true,
            message,
        ...
        Ok(error == BLST_ERROR::BLST_SUCCESS)
    }
    ...
    fn verify_bls_aggregate(
        ...
        Err(DispatchError::Other(
            "BLS verification requires the std feature",
        ))
    }

    // Caller-facing: pallet/relayer supplies the externally-attested expected root / fee.
    #[allow(dead_code)]
    ...
    fn verify_state_root(
        ...
        if state_root != expected_state_root {
            return Err(DispatchError::Other("State root mismatch"));
        }
        Ok(())
    }
    ...
    fn verify_canonical_fee(&self, fee: u64) -> Result<(), DispatchError> {
        if fee != self.canonical_fee {
            return Err(DispatchError::Other("Canonical fee mismatch"));
        }
        Ok(())
    }
}
...
impl VmFinalityVerifier for EvmFinalityVerifier {
    ...
    fn verify_finality_proof(
        ...
        let proof = EvmFinalityProof::decode(&mut &proof_bytes[..])
            .map_err(|_| DispatchError::Other("Failed to decode EVM finality proof"))?;
        ...
        let checkpoint_age = self
            .current_epoch
            .saturating_sub(proof.checkpoint_block.finality_epoch);
        ...
        let active_validators: Vec<_> = validator_set
            .iter()
        ...
        let mut bitfield = vec![0u8; active_validators.len().div_ceil(8)];
        ...
        let block_distance = proof
            .finalized_block
            .block_number
            .saturating_sub(proof.checkpoint_block.block_number);
        if proof.finalized_block.parent_hash != proof.checkpoint_block.block_hash
            && block_distance > 1
        {
            return Ok(FinalityVerificationResult {
                is_valid: false,
...
impl SvmFinalityVerifier {
    ...
    fn verify_ed25519(
        ...
        if signature.len() != 64 {
            return Err(DispatchError::Other("Ed25519 signature must be 64 bytes"));
        }
        if public_key.len() != 32 {
            return Err(DispatchError::Other("Ed25519 public key must be 32 bytes"));
        }
        ...
        #[cfg(feature = "std")]
        {
            ...
            // ed25519-dalek 2.x `from_bytes` takes fixed-size arrays. The slice
            // lengths were validated above (64 / 32 bytes), so the fallible
            // conversion cannot fail; we treat failure as verification-failed.
            ...
            // Create signature from bytes. `ed25519::Signature::from_bytes`
            // (re-exported as `ed25519_dalek::Signature::from_bytes`) is
            // infallible and takes a `&[u8; 64]`. RFC 8032 verification hashes
            // the message internally; no pre-hashing is done here.
        ...
        #[cfg(not(feature = "std"))]
        {
            ...
            // This is a known limitation - use std feature for actual signature verification
            Err(DispatchError::Other(
                "Ed25519 verification requires the std feature",
            ))
        }
    }
    ...
    fn verify_lockout_chain(&self, lockouts: &[LockoutInfo]) -> Result<bool, DispatchError> {
        ...
        for lockout in lockouts {
            if lockout.lockout_depth < last_depth && last_depth > 0 {
                return Err(DispatchError::Other("Invalid lockout sequence"));
            }
            last_depth = lockout.lockout_depth;
        }
        Ok(true)
...
impl VmFinalityVerifier for SvmFinalityVerifier {
    ...
    fn verify_finality_proof(
        ...
        let proof = SvmFinalityProof::decode(&mut &proof_bytes[..])
            .map_err(|_| DispatchError::Other("Failed to decode SVM finality proof"))?;
        ...
        if signed_count < threshold {
            return Ok(FinalityVerificationResult {
                ...
                finalized_block: None,
                error_message: Some(
                    format!("Insufficient votes: {} < {}", signed_count, threshold)
                        .as_bytes()
                        .to_vec(),
                ),
                verification_data,
        ...
        steps_passed.push(
            format!("Vote threshold verified: {} >= {}", signed_count, threshold)
                .as_bytes()
                .to_vec(),
        );
        ...
        for vote in &proof.vote_signatures {
            ...
            let validator = validator_set
                ...
                .ok_or(DispatchError::Other("Validator index out of range"))?;
            ...
            if validator.is_slashed || !validator.is_active {
                return Ok(FinalityVerificationResult {
                    ...
                    finalized_block: None,
                    error_message: Some(
                        format!("Validator {} is slashed or inactive", vote.validator_index)
                            .as_bytes()
                            .to_vec(),
                    ),
                    verification_data,
        ...
        if valid_sigs < threshold {
            return Ok(FinalityVerificationResult {
                ...
                finalized_block: None,
                error_message: Some(
                    format!("Invalid signatures: {} < {}", valid_sigs, threshold)
                        .as_bytes()
                        .to_vec(),
                ),
                verification_data,
        ...
        steps_passed.push(
            format!("All {} signatures verified", valid_sigs)
                .as_bytes()
                .to_vec(),
        );
...
impl CrossVmFinalityVerifier {
    ...
    ///
    /// `current_time` (Unix seconds) is supplied by the caller so this verifier
    /// stays runtime-context-free (no `sp_io`) and is testable without a host.
    pub fn verify_cross_vm_proof(
        ...
        if let Some(ref evm_proof) = proof.evm_proof {
            let evm_result =
                self.evm_verifier
                    .verify_finality_proof(&evm_proof.encode(), evm_validators, 0)?;
            if !evm_result.is_valid {
                return Ok(FinalityVerificationResult {
                    is_valid: false,
                    finalized_block: None,
                    error_message: Some(
                        format!("EVM verification failed: {:?}", evm_result.error_message)
                            .as_bytes()
                            .to_vec(),
                    ),
        ...
        if let Some(ref svm_proof) = proof.svm_proof {
            let svm_result =
                self.svm_verifier
                    .verify_finality_proof(&svm_proof.encode(), svm_validators, 0)?;
            if !svm_result.is_valid {
                return Ok(FinalityVerificationResult {
                    is_valid: false,
                    finalized_block: None,
                    error_message: Some(
                        format!("SVM verification failed: {:?}", svm_result.error_message)
                            .as_bytes()
                            .to_vec(),
                    ),
        ...
        if let (Some(evm), Some(svm)) = (evm_block.as_ref(), svm_block.as_ref()) {
            ...
            if age_diff > proof.min_block_age {
                return Ok(FinalityVerificationResult {
                    ...
                    finalized_block: None,
                    error_message: Some(
                        format!("Cross-VM timestamp mismatch: {} seconds apart", age_diff)
                            .as_bytes()
                            .to_vec(),
                    ),
                    verification_data,
