...
/// Derive a deterministic, domain-separated balance-slot storage key for an
/// account address (20-byte EVM or 32-byte SVM). Namespace is `"x3/bridge/bal"`,
/// so the key is reproducible by any consumer and is not H256::zero().
fn balance_slot_key(address: &[u8]) -> H256 {
    let mut ctx = Sha256::new();
    ctx.update(b"x3/bridge/bal");
    ctx.update(address);
    let d = ctx.finalize();
    H256::from_slice(&d)
}
        ...
        guard
            ...
            .map(|(addr, entry)| {
                ...
                StateChange {
                    ...
                    // from the account address (the overlay is keyed by address
    ...
    C::Api: AtlasKernelRuntimeApi<Block, AccountId32, u128, u32>,
{
    ...
    fn execute_x3vm_tx(
        ...
        // Genuine source-chain state anchor = the observed X3 client head this
        // dispatcher is actually operating on (not H256::zero()). This is a
        // real, reproducible value and binds the receipt to the chain state the
        // runtime observed at execution time.
        let source_state_root: H256 = {
            let h = self.best_hash();
            let mut sr = H256::zero();
            sr.as_bytes_mut().copy_from_slice(h.as_ref());
            sr
        };

        if call.target != VmId::X3Vm {
            return Ok(CrossVmReceipt {
                call_hash: call.call_hash(&source_state_root),
                source_state_root,
                target_state_root: source_state_root,
        ...
        match bridge.execute(call.payload.as_ref(), function_index, &[]) {
            Ok(result) => Ok(CrossVmReceipt {
                call_hash: call.call_hash(&source_state_root),
                source_state_root,
                // The X3VM dispatcher produces no real VM post-state root at
                // this seam; carry the observed source anchor rather than a
                // fabricated target root. Outcome is conveyed by status.
                target_state_root: source_state_root,
...
mod tests {
    ...
    fn runtime_dispatcher_executes_x3vm_with_client_backed_escrow_config() {
        ...
        // source state root is derived from the observed client head
        // (H256::repeat_byte(0x42) in the harness), not H256::zero().
        let expected_root = H256::repeat_byte(0x42);
        assert_eq!(receipt.source_state_root, expected_root);
        assert_eq!(receipt.target_state_root, expected_root);
