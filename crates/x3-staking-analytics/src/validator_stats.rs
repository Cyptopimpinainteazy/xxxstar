...
mod tests {
    ...
    fn test_overall_score() {
        ...
        manager.register_validator("val1", "Validator One", 5.0, 1000000);
        manager.update_performance("val1", 950, 50, 500).unwrap();
        // A well-nominated, high-uptime, low-commission validator should score
        // highly. With 0 nominators the nominator component (0.2 weight) is zero
        // and even a perfect validator scores low, so fund a realistic base.
        manager.update_nominator_count("val1", 600).unwrap();
