//! # FuelVM HTLC Adapter
//!
//! Adapter for FuelVM chains. Implements [`X3VmAdapter`] with
//! mock/placeholder proof structures.
//!
//! In production, [`lock`] would deploy or call a Fuel predicate with the
...
impl FuelNetwork {
    pub fn name(&self) -> &'static str {
        match self {
            FuelNetwork::Mainnet => "fuel-mainnet",
            FuelNetwork::Testnet => "fuel-testnet",
        }
    }
}

/// A Fuel predicate that enforces an HTLC.
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
pub struct FuelPredicate {
    pub predicate_code: Vec<u8>,
    pub hashlock: [u8; 32],
    pub receiver: Vec<u8>,
...
/// Adapter for FuelVM chains.
///
/// Uses mock/placeholder proof data. In real operation this would connect to a
/// Fuel node via GraphQL API and interact with Fuel predicates/contracts.
///
/// For stateful double-claim/double-refund enforcement, use
/// [`StatefulFuelAdapter`].
#[derive(Debug, Clone)]
pub struct FuelHtlcAdapter {
    /// Chain identifier (e.g. "fuel-mainnet", "fuel-testnet").
...
impl FuelHtlcAdapter {
    /// Create a new adapter for the given chain identifier.
    ///
    /// Example chain IDs: `"fuel-mainnet"`, `"fuel-testnet"`.
    pub fn new(chain_id: ChainId, network: FuelNetwork) -> Self {
        Self {
            chain_id,
            network,
            rpc_url: None,
    ...
    /// Generate a mock Fuel predicate/contract address.
    fn mock_address(chain_id: &str) -> String {
        let mut hasher = Sha256::new();
        hasher.update(b"x3-fuel-htlc:");
...
impl X3VmAdapter for FuelHtlcAdapter {
    ...
    fn supported_chains(&self) -> Vec<ChainId> {
        vec!["fuel-mainnet".into(), "fuel-testnet".into()]
    }

    fn supported_assets(&self) -> Vec<AssetId> {
        vec!["ETH".into(), "FUEL".into(), "USDC".into()]
    }
    ...
    fn lock(&self, intent: &AtomicIntent) -> Result<LockProof, SwapError> {
        ...
        Ok(LockProof {
            ...
            raw_proof: vec![0x66, 0x75, 0x65, 0x01], // "fue\x01" - mock proof
        })
    }
    ...
    fn claim(&self, intent_id: IntentId, preimage: [u8; 32]) -> Result<ClaimProof, SwapError> {
        ...
        Ok(ClaimProof {
            ...
            block_number,
            block_hash: hex::encode(Sha256::digest(block_number.to_le_bytes())),
            raw_proof: vec![0x66, 0x75, 0x65, 0x02], // "fue\x02" - mock proof
        })
    }
    ...
    fn refund(&self, intent_id: IntentId) -> Result<RefundProof, SwapError> {
        ...
        Ok(RefundProof {
            ...
            block_number,
            block_hash: hex::encode(Sha256::digest(block_number.to_le_bytes())),
            raw_proof: vec![0x66, 0x75, 0x65, 0x03], // "fue\x03" - mock proof
        })
    }
    ...
    fn verify_lock(&self, proof: &LockProof) -> Result<bool, SwapError> {
        if proof.vm_type != VmType::FuelVm {
            return Ok(false);
        }
    ...
    fn estimate_fee(&self, _intent: &AtomicIntent) -> Result<FeeEstimate, SwapError> {
        // ~0.001 ETH equivalent for Fuel execution
        Ok(FeeEstimate {
            chain_id: self.chain_id.clone(),
            vm_type: VmType::FuelVm,
            native_fee: 1_000_000_000_000_000, // 0.001 ETH in wei
            gas_units: 0,
            gas_price: 0,
            estimated_usd: 1.50,
        })
    ...
    fn finality_status(&self, tx_id: &TxId) -> Result<FinalityProof, SwapError> {
        // Fuel UTXO model: ~1 block finality
        ...
        Ok(FinalityProof {
            ...
            finalized,
            finality_source: "fuel-utxo".into(),
    ...
    fn chain_health(&self) -> Result<ChainHealth, SwapError> {
        Ok(ChainHealth {
            ...
            finalized_block: self.last_block,
            block_delay_ms: 1_000,    // ~1s Fuel block time
            finality_delay_ms: 1_000, // ~1 block finality
    ...
    fn readiness_score(&self) -> AdapterReadinessScore {
        AdapterReadinessScore {
            adapter_name: "x3-adapter-fuelvm",
            vm_type: VmType::FuelVm,
            interface_implemented: true,
            lock_path: true,
            claim_path: true,
            refund_path: true,
            event_proof_extraction: false, // needs actual Fuel event extraction
            finality_proof: true,
...
/// A stateful wrapper around [`FuelHtlcAdapter`] that tracks lock state
/// in order to reject double claims and double refunds.
#[derive(Debug, Clone)]
pub struct StatefulFuelAdapter {
    pub inner: FuelHtlcAdapter,
    locks: Vec<InternalLock>,
}

impl StatefulFuelAdapter {
    pub fn new(chain_id: ChainId, network: FuelNetwork) -> Self {
        Self {
            inner: FuelHtlcAdapter::new(chain_id, network),
            locks: Vec::new(),
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_fuel_network_name() {
        assert_eq!(FuelNetwork::Mainnet.name(), "fuel-mainnet");
        assert_eq!(FuelNetwork::Testnet.name(), "fuel-testnet");
    }

    #[test]
    fn test_fuel_network_equality() {
        assert_eq!(FuelNetwork::Mainnet, FuelNetwork::Mainnet);
        assert_ne!(FuelNetwork::Mainnet, FuelNetwork::Testnet);
    }

    #[test]
    fn test_fuel_predicate() {
        let predicate = FuelPredicate {
            predicate_code: vec![0x00, 0x01],
            hashlock: [0xabu8; 32],
            receiver: vec![0x02],
            refund_address: vec![0x03],
            timeout: 1000,
        };
        assert_eq!(predicate.hashlock[0], 0xab);
        assert_eq!(predicate.timeout, 1000);
    }
    ...
    fn test_adapter_identity() {
        let adapter = FuelHtlcAdapter::new("fuel-mainnet".into(), FuelNetwork::Mainnet);

        assert_eq!(adapter.vm_type(), VmType::FuelVm);
        assert_eq!(adapter.adapter_name(), "x3-adapter-fuelvm");

        let chains = adapter.supported_chains();
        assert!(chains.contains(&"fuel-mainnet".into()));
        assert!(chains.contains(&"fuel-testnet".into()));

        let assets = adapter.supported_assets();
        assert!(assets.contains(&"ETH".into()));
        assert!(assets.contains(&"FUEL".into()));
    ...
    fn test_adapter_name_const() {
        ...
        assert_eq!(name, "x3-adapter-fuelvm");
    }
    ...
    fn test_lock_creates_proof() {
        ...
        assert_eq!(proof.vm_type, VmType::FuelVm);
    ...
    fn test_lock_differs_per_intent() {
        let adapter = FuelHtlcAdapter::new("fuel-testnet".into(), FuelNetwork::Testnet);
    ...
    fn test_finality_status() {
        ...
        assert_eq!(fp.chain_id, "fuel-mainnet");
        assert_eq!(fp.vm_type, VmType::FuelVm);
        assert_eq!(fp.finality_source, "fuel-utxo");
    }
    ...
    fn test_chain_health() {
        ...
        assert_eq!(health.chain_id, "fuel-testnet");
        assert_eq!(health.vm_type, VmType::FuelVm);
    ...
    fn test_estimate_fee() {
        ...
        assert_eq!(fee.chain_id, "fuel-mainnet");
        assert_eq!(fee.vm_type, VmType::FuelVm);
    ...
    fn test_readiness_score() {
        ...
        assert_eq!(score.adapter_name, "x3-adapter-fuelvm");
        assert_eq!(score.vm_type, VmType::FuelVm);
    ...
    fn test_double_claim_rejected() {
        let mut adapter = StatefulFuelAdapter::new("fuel-mainnet".into(), FuelNetwork::Mainnet);
    ...
    fn test_double_refund_rejected() {
        let mut adapter = StatefulFuelAdapter::new("fuel-testnet".into(), FuelNetwork::Testnet);
    ...
    #[test]
    fn test_set_rpc() {
        let mut adapter = FuelHtlcAdapter::new("fuel-mainnet".into(), FuelNetwork::Mainnet);
        assert!(adapter.rpc_url.is_none());

        adapter.set_rpc("https://fuel-mainnet.rpc:8080");

        assert_eq!(
            adapter.rpc_url.as_deref(),
            Some("https://fuel-mainnet.rpc:8080")
        );
    }

    #[test]
    fn test_adapter_chain_id_independence() {
        let mainnet = FuelHtlcAdapter::new("fuel-mainnet".into(), FuelNetwork::Mainnet);
        let testnet = FuelHtlcAdapter::new("fuel-testnet".into(), FuelNetwork::Testnet);

        assert_eq!(mainnet.vm_type(), VmType::FuelVm);
        assert_eq!(testnet.vm_type(), VmType::FuelVm);
