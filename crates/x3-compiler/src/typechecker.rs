//! X3 language type checker with asset symbol validation
//!
//! Adds DEX/bridge registry checks for asset symbols
//!
use crate::parser::{BinOp, Expr, FnDecl, SourceFile, Stmt, TypeExpr};
use crate::registry::Registry;
use std::collections::HashMap;

/// A type error from the type checker.
#[derive(Debug, PartialEq, Eq, Clone)]
pub struct TypeError {
    pub message: String,
    pub location: String,
}

impl TypeError {
    pub fn new(location: impl Into<String>, message: impl Into<String>) -> Self {
        Self {
            location: location.into(),
            message: message.into(),
        }
    }
}

/// The type environment: maps names to their types.
pub type TypeEnv = HashMap<String, TypeExpr>;

/// Type-check the binary operator application and return the result type.
pub fn typecheck_binop(
    op: &BinOp,
    lhs: &TypeExpr,
    rhs: &TypeExpr,
    registry: &Registry,
) -> Result<TypeExpr, TypeError> {
    match op {
        BinOp::Add | BinOp::Sub | BinOp::Mul | BinOp::Div => {
            if lhs == &TypeExpr::I64 && rhs == &TypeExpr::I64 {
                Ok(TypeExpr::I64)
            } else if lhs == &TypeExpr::U64 && rhs == &TypeExpr::U64 {
                Ok(TypeExpr::U64)
            } else {
                Err(TypeError::new(
                    "binop",
                    format!("type mismatch: {lhs:?} op {rhs:?}"),
                ))
            }
        }
        BinOp::Eq | BinOp::Ne | BinOp::Lt | BinOp::Gt => {
            if lhs == rhs {
                Ok(TypeExpr::Bool)
            } else {
                Err(TypeError::new(
                    "binop",
                    format!("cannot compare {lhs:?} with {rhs:?}"),
                ))
            }
        }
    }
}

/// Infer the type of an expression given a type environment and registry.
pub fn infer_expr(expr: &Expr, env: &TypeEnv, registry: &Registry) -> Result<TypeExpr, TypeError> {
    match expr {
        Expr::IntLit(_) => Ok(TypeExpr::I64),
        Expr::BoolLit(_) => Ok(TypeExpr::Bool),
        Expr::StrLit(_) => Ok(TypeExpr::Str),
        Expr::Ident(name) => {
            if let Some(ty) = env.get(name) {
                Ok(ty.clone())
            } else {
                // Check if this is an asset symbol
                if registry.is_valid_symbol(name) {
                    Ok(TypeExpr::Named(name.to_string()))
                } else {
                    Err(TypeError::new(
                        name,
                        format!("undefined variable '{name}' or invalid asset symbol"),
                    ))
                }
            }
        }
        Expr::BinOp { op, lhs, rhs } => {
            let lt = infer_expr(lhs, env, registry)?;
            let rt = infer_expr(rhs, env, registry)?;
            typecheck_binop(op, &lt, &rt, registry)
        }
        Expr::Call { callee, args: _ } => {
            // Simplified: calls return Unit (full inference requires function table)
            Ok(env.get(callee).cloned().unwrap_or(TypeExpr::Unit))
        }
        Expr::Block(stmts) => {
            let mut local_env = env.clone();
            let mut last = TypeExpr::Unit;
            for stmt in stmts {
                last = typecheck_stmt(stmt, &mut local_env, registry)?;
            }
            Ok(last)
        }
        Expr::If {
            cond,
            then_branch,
            else_branch,
        } => {
            let ct = infer_expr(cond, env, registry)?;
            if ct != TypeExpr::Bool {
                return Err(TypeError::new("if", "condition must be Bool"));
            }
            let tt = infer_expr(then_branch, env, registry)?;
            if let Some(eb) = else_branch {
                let et = infer_expr(eb, env, registry)?;
                if tt != et {
                    return Err(TypeError::new("if", "then/else branch types must match"));
                }
            }
            Ok(tt)
        }
    }
}

/// Type-check a statement, updating the environment and returning the "value type".
pub fn typecheck_stmt(
    stmt: &Stmt,
    env: &mut TypeEnv,
    registry: &Registry,
) -> Result<TypeExpr, TypeError> {
    match stmt {
        Stmt::Let { name, ty, value } => {
            let inferred = infer_expr(value, env, registry)?;
            if let Some(expected) = ty {
                if &inferred != expected {
                    return Err(TypeError::new(
                        name,
                        format!("expected {expected:?}, got {inferred:?}"),
                    ));
                }
            }
            env.insert(name.clone(), inferred);
            Ok(TypeExpr::Unit)
        }
        Stmt::Return(expr) => {
            if let Some(e) = expr {
                infer_expr(e, env, registry)
            } else {
                Ok(TypeExpr::Unit)
            }
        }
        Stmt::Expr(e) => infer_expr(e, env, registry),
        Stmt::While { cond, body } => {
            let ct = infer_expr(cond, env, registry)?;
            if ct != TypeExpr::Bool {
                return Err(TypeError::new("while", "condition must be Bool"));
            }
            infer_expr(body, env, registry)?;
            Ok(TypeExpr::Unit)
        }
    }
}

/// Type-check a function declaration.
pub fn typecheck_fn(
    func: &FnDecl,
    global: &TypeEnv,
    registry: &Registry,
) -> Result<(), Vec<TypeError>> {
    let mut env = global.clone();
    for p in &func.params {
        env.insert(p.name.clone(), p.ty.clone());
    }
    let mut errors = Vec::new();
    for stmt in &func.body {
        if let Err(e) = typecheck_stmt(stmt, &mut env, registry) {
            errors.push(e);
        }
    }
    if errors.is_empty() {
        Ok(())
    } else {
        Err(errors)
    }
}

/// Type-check an entire source file.
pub fn typecheck_file(file: &SourceFile, registry: &Registry) -> Vec<TypeError> {
    let global: TypeEnv = file
        .functions
        .iter()
        .map(|f| (f.name.clone(), f.return_ty.clone()))
        .collect();
    let mut all_errors = Vec::new();
    for func in &file.functions {
        if let Err(mut errs) = typecheck_fn(func, &global, registry) {
            all_errors.append(&mut errs);
        }
    }
    all_errors
}

#[cfg(test)]
mod tests {
    use super::*;
    use crate::parser::{FnDecl, Param, TypeExpr};

    fn empty_env() -> TypeEnv {
        TypeEnv::new()
    }

    #[test]
    fn test_asset_symbol_validation() {
        let registry = Registry::new();
        let env = empty_env();

        // Valid asset symbol
        let result = infer_expr(&Expr::Ident("USDC".to_string()), &env, &registry);
        assert!(result.is_ok());

        // Invalid asset symbol
        let result = infer_expr(&Expr::Ident("XRP".to_string()), &env, &registry);
        assert!(result.is_err());
    }

    // Other existing tests...
}
