...
fn assess_ecosystem_quality(workspace: &Path, claim_id: &str) -> ProofResult {
    let started = Instant::now();
    let checks: [(&str, &str); 4] = [
        ("README.md", "Root project README exists"),
        ("docs/README.md", "Docs index exists"),
        ("MASTER_STATUS.md", "Master status tracker exists"),
        ("ROADMAP.md", "Top-level roadmap exists"),
    ];
    ...
    for (rel, label) in checks {
        ...
        if ok {
            ...
            failed_checks.push(format!("Missing ecosystem-quality artifact: {}", rel));
    ...
    ProofResult {
        ...
        claim: "Ecosystem quality baseline artifacts are present".to_string(),
        status,
...
pub async fn verify_claim(workspace: &Path, claim_id: &str, _verbose: bool) -> Result<ProofResult> {
    Ok(assess_ecosystem_quality(workspace, claim_id))
}

pub async fn run_proofs(workspace: &Path, _verbose: bool) -> Result<ProofResult> {
    Ok(assess_ecosystem_quality(
        workspace,
        "x3.ecosystem_quality.full_proof",
    ))
}
