...
async fn assess_x3vm(workspace: &Path, claim_id: &str) -> Result<ProofResult> {
    let started = Instant::now();
    let checks: [(&str, &str); 4] = [
        ("crates/x3-vm/Cargo.toml", "x3-vm crate manifest exists"),
        ("crates/x3-vm/src/vm.rs", "x3-vm execution core exists"),
        (
            "crates/x3-vm/src/bridge.rs",
            "x3-vm bridge integration module exists",
        ),
        (
            "proof/receipts/claims/x3.x3vm.determinism.receipt.json",
            "x3vm determinism receipt exists",
        ),
    ];
    ...
    for (rel, label) in checks {
        ...
        if ok {
            ...
            failed_checks.push(format!("Missing x3vm artifact: {}", rel));
            missing_proofs.push(format!("Add or restore {}", rel));
        }
    }

    // Executable gate: x3-vm crate must resolve and type-check.
    let vm_manifest = workspace.join("crates/x3-vm/Cargo.toml");
    if vm_manifest.exists() {
        let cmd = "cargo check -p x3-vm --quiet";
            ...
            .await
        {
            Ok(output) if output.status.success() => {
                passed_checks.push("x3-vm executable check passed".to_string());
                evidence.insert("x3vm_check".to_string(), "true".to_string());
            }
            Ok(output) => {
                let stderr = String::from_utf8_lossy(&output.stderr);
                failed_checks.push(format!(
                    "x3-vm executable check failed: {}",
                    stderr
                        .lines()
                        .find(|l| !l.trim().is_empty())
                        .unwrap_or("unknown error")
                ));
                evidence.insert("x3vm_check".to_string(), "false".to_string());
            }
            Err(e) => {
                failed_checks.push(format!("x3-vm executable check could not run: {}", e));
                evidence.insert("x3vm_check".to_string(), "false".to_string());
            }
        }
    }
    ...
    Ok(ProofResult {
        ...
        claim: "X3VM determinism and execution artifacts are present".to_string(),
        status,
...
pub async fn verify_claim(workspace: &Path, claim_id: &str, _verbose: bool) -> Result<ProofResult> {
    assess_x3vm(workspace, claim_id).await
}

pub async fn run_proofs(workspace: &Path, _verbose: bool) -> Result<ProofResult> {
    assess_x3vm(workspace, "x3.x3vm.full_proof").await
}
