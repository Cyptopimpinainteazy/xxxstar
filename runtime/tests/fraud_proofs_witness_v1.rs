//! Fraud Proof Witness v1 — Integration tests (Rust integration test for `runtime` crate).
//!
//! Invariants tested:
...
use x3_chain_runtime::fraud_proofs::witness_v1::{
...
fn tx_count_mismatch_rejected() {
    let mut w = build_valid_witness(4, 1);
    w.tx_count = Compact(5); // claim 5 but only 4
    let enc = w.encode();
    let err = SchedulerWitnessV1::decode_and_validate(&enc, 1, 1000).unwrap_err();
    assert_eq!(err, WitnessError::TxCountMismatch);
}
