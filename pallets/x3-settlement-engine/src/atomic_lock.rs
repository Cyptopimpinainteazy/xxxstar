...
/// The settlement pallet provides the actual escrow account (typically the maker or a dedicated escrow)
/// No automatic derivation needed - caller is responsible for providing the correct escrow account
pub struct EscrowAccount;

impl EscrowAccount {
    /// Derive escrow address bytes for a given intent
    /// Useful for off-chain escrow address generation
    pub fn derive_bytes(intent_id: &[u8; 32], nonce: u32) -> [u8; 32] {
        // Deterministic derivation for escrow accounts
        // Used for off-chain address computation but not required in pallet
        let mut data = Vec::with_capacity(40);
        data.extend_from_slice(intent_id);
        data.extend_from_slice(b"escrow");
        data.extend_from_slice(&nonce.to_le_bytes());
        // Simple hash - in pallet context, actual AccountId is passed in
        let mut result = [0u8; 32];
        result.copy_from_slice(&data.as_slice()[..32.min(data.len())]);
        result
    }
}
    ...
    T::AccountId: Clone + Eq,
{
    ...
    // NOTE: lock_for_prepare is not used by settlement pallet
    // Settlement pallet directly uses AtomicLock::new_prepare and manages locks via storage
    // This registry is kept for potential future use or testing but requires escrow account
    // to be passed in rather than derived
