...
pub mod pallet {
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn fetch_asset_metadata(asset_id: &AssetId) -> Result<AssetStatus, &'static str> {
            T::Registry::status(asset_id).ok_or("Asset not found in registry")
        }

        pub fn enforce_supply_policy(asset_id: &AssetId) -> Result<(), &'static str> {
            if let Some(policy) = T::Registry::supply_policy(asset_id) {
                match policy {
                    SupplyPolicy::NativeMintBurn => Ok(()),
                    _ => Err("Unsupported supply policy"),
                }
            } else {
                Err("Supply policy not found")
            }
        }
    }
