...
//! X3 control plane — freeze / pause / quarantine per domain.
//!
//! Every `execute_control_action` call is authority-checked against the origin
//! that actually signed it (see `ensure_authority`). Until 2026-09-19 it was
//! not: the `Root` and `Governance` arms manufactured a root origin and checked
//! *that*, so any signed account could freeze the chain, an HTLC domain or the
//! proof relay by naming `ControlAuthority::Governance`.
...
pub mod pallet {
    ...
    pub struct ControlRecord<T: Config> {
        ...
        pub cooldown_until: BlockNumberFor<T>,
    }

    /// Storage for control records
    #[pallet::storage]
    pub type ControlRecords<T: Config> =
        StorageMap<_, Blake2_128Concat, ControlDomain, ControlRecord<T>, OptionQuery>;
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn execute_control_action(
            ...
            // The authority check needs the *origin*, not just the account: it
            // used to be handed `OriginFor::<T>::root()`, which this function
            // manufactured for the Root and Governance arms — so any signed
            // account could claim governance authority and freeze the chain.
            Self::authorize(origin, authority)?;
            Self::ensure_action_allowed(authority, action)?;

            // Check cooldown
            let record = ControlRecords::<T>::get(domain).unwrap_or_default();
            let current_block = <frame_system::Pallet<T>>::block_number();
            ensure!(
                current_block >= record.cooldown_until,
                Error::<T>::ActionRateLimited
            );

            // Validate transition
            let previous_state = record.state;
            let new_state = Self::compute_new_state(previous_state, action)?;
            ensure!(
            ...
            // Emit events
            Self::deposit_event(Event::ControlActionExecuted {
                domain,
                action,
                authority,
            });
    ...
    /// Helper functions
    impl<T: Config> Pallet<T> {
        /// Check the authority the caller claims against the origin that signed
        /// the call.
        ///
        ...
        fn authorize(origin: OriginFor<T>, authority: ControlAuthority) -> DispatchResult {
            match authority {
                ControlAuthority::Root | ControlAuthority::Governance => {
                    EnsureGovernance::<T>::ensure_origin(origin)?;
                }
        ...
        /// Which actions an authority level may request.
        ///
        /// The action itself has to be gated, not just the caller's identity:
        /// the state table accepts `Active -> Frozen` from any authority, so an
        /// unprivileged operator could otherwise freeze the chain outright.
        /// `Pause` is the reversible action and is the most an operator or the
        /// emergency committee needs; `Freeze`/`Thaw` stay with root/governance.
        fn ensure_action_allowed(
            ...
            let allowed = match authority {
                ControlAuthority::Root | ControlAuthority::Governance => true,
                ControlAuthority::EmergencyCommittee => {
                    matches!(action, Pause | Resume | Quarantine | Release)
                }
                ControlAuthority::Operator => matches!(action, Pause | Resume),
            };
            ensure!(allowed, Error::<T>::InsufficientAuthority);
            Ok(())
        }
        ...
        fn get_cooldown_period(authority: ControlAuthority) -> BlockNumberFor<T> {
            match authority {
