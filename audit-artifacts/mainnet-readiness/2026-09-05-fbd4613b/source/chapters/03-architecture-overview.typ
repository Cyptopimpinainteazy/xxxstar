...
// NOTE: cetz's `draw` module exports its own style-setter named `fill`
// (draw.fill(color)). Importing `draw: *` inside a helper function shadows
// any local parameter literally named `fill`, silently turning a color
// argument into that function instead. Parameters below are named
// `fill-color` / `stroke-color` to avoid this collision.
#let node-box(pos, w, h, label, fill-color: white, stroke-color: rgb("#1a1a2e"), text-size: 7.6pt, text-color: rgb("#1a1a2e")) = {
  import draw: *
  rect(pos, (pos.at(0) + w, pos.at(1) - h), fill: fill-color, stroke: 0.9pt + stroke-color)
...
#figure(
  canvas(length: 1cm, {
    import draw: *
    node-box((0, 0), 5, 1.1, [x3-chain-node\ (node/src/service.rs)], fill-color: rgb("#e8eef5"), stroke-color: rgb("#1e3a5f"))
    node-box((-5.6, -2.4), 3.6, 1, [Aura\ block production])
    node-box((-1.4, -2.4), 3.6, 1, [GRANDPA\ finality voter])
    node-box((2.8, -2.4), 3.6, 1, [sc-transaction-pool\ (tx pool)])
    arrow((-4, -1.1), (-3.8, -2.4))
    arrow((-1, -1.1), (-1.1, -2.4))
    arrow((2, -1.1), (2.8, -2.4))

    node-box((-7.2, -5), 3.4, 1, [pallet-x3-consensus\ (validator set, slash)], fill-color: rgb("#fbe9e7"), stroke-color: rgb("#8c1c13"))
    node-box((-3.4, -5), 3.4, 1, [pallet-x3-cross-vm-router\ (6-route matrix)])
    node-box((0.4, -5), 3.4, 1, [pallet-x3-supply-ledger\ (canonical supply)])
    node-box((4.2, -5), 3.4, 1, [pallet-x3-settlement-engine\ (atomic escrow)])
    ...
    node-box((-3.4, -7.6), 3.4, 1, [pallet-x3-dex\ (AMM, f64 pricing — see HIGH-02)], fill-color: rgb("#faf3d9"), stroke-color: rgb("#8a6d00"))
    node-box((0.4, -7.6), 3.4, 1, [pallet-x3-atomic-kernel\ (economic-halt gate)])
    node-box((4.2, -7.6), 3.4, 1, [pallet-x3-cross-vm-router:\ ExternalBridgesEnabled gate], fill-color: rgb("#e8f3e8"), stroke-color: rgb("#2e6b32"))
...
#figure(
  canvas(length: 1cm, {
    import draw: *
    node-box((0, 0), 4.4, 1, [Block proposed\ (Aura leader slot)])
    node-box((5.2, 0), 4.4, 1, [GRANDPA vote\ round])
    node-box((10.4, 0), 4.4, 1, [Finalized block], fill-color: rgb("#e8f3e8"), stroke-color: rgb("#2e6b32"))
    arrow((4.4, -0.5), (5.2, -0.5))
    arrow((9.6, -0.5), (10.4, -0.5))

    node-box((0, -2.6), 5.6, 1.3, [Equivocation observed\ → EquivocationProof\ → check_equivocation_proof()\ (real cryptographic check)], fill-color: rgb("#e8f3e8"), stroke-color: rgb("#2e6b32"), text-size: 7pt)
    node-box((6.6, -2.6), 5.6, 1.3, [report_misbehavior(origin, validator, reason)\ ensure_signed() only — NO PROOF\ → slash_validator() immediately], fill-color: rgb("#fbe9e7"), stroke-color: rgb("#8c1c13"), text-size: 7pt)

    arrow((2, -1.1), (2.5, -2.6))
    line((6, -2), (9, -1.1), stroke: (paint: rgb("#8c1c13"), thickness: 1pt, dash: "dashed"), mark: (end: ">", fill: rgb("#8c1c13"), size: 0.16))

    node-box((3.4, -5.2), 5.6, 1, [slash_validator() — stake reduced])
    arrow((2.5, -3.9), (5, -5.2))
    arrow((9.2, -3.9), (7, -5.2))
  }),
...
#figure(
  canvas(length: 1cm, {
    import draw: *
    node-box((0, 0), 5, 1.3, [External chain (EVM / SVM)\ compile-time excluded from\ mainnet-rc1 by design], stroke-color: rgb("#5a5a6e"))
    node-box((0, -2.4), 5, 1.3, [x3-relayer\ proof envelope signer:\ defaults to dev key \/\/Alice], fill-color: rgb("#fdefe2"), stroke-color: rgb("#a8460a"), text-size: 7pt)
    node-box((0, -4.8), 5, 1, [node RPC:\ x3_submitCrossVmTransaction])
    node-box((0, -7.2), 5, 1.3, [ExternalBridgesEnabled gate\ (real, dispatch-time check,\ auto-trips false on violation)], fill-color: rgb("#e8f3e8"), stroke-color: rgb("#2e6b32"), text-size: 7pt)
    node-box((6.5, -7.2), 5, 1.3, [pallet-x3-supply-ledger\ canonical_supply invariant])
    ...
    content((-2.2, 1.9), text(size: 7pt, fill: rgb("#7a7a8a"))[← TRUST BOUNDARY], anchor: "center")
  }),
